...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    std::cout
        << "Invalid parameter. Please provide a valid integer (0, 1, 2, or 3)."
        << std::endl;
    return 1;
  }

  if (parameter < 0 || parameter > 3) {
    std::cout << "Invalid parameter. Please provide a value between 0 and 3 "
                 "(inclusive)."
              << std::endl;
    return 1;
  }
  ...
  if (parameter == 1) {
    ...
    std::cout << "Running sanitizer checks on generated object files..."
              << std::endl;
  } else if (parameter == 2) {
    ...
    try {
      ...
      if (!foundFiles) {
        std::cout << "No object files found in ../object directory to process."
                  << std::endl;
      }
