#ifndef LLM_TOKENS_OPTIONS_HPP
#define LLM_TOKENS_OPTIONS_HPP

#include <iostream>
#include <chrono>
#include <random>
#include <string>
#include <vector>
#include <array>
#include <memory>
#include <stdexcept>
#include <cstdio>
#include <unordered_set>
#include <sstream>

class LLMTokensOption {
private:
  std::vector<std::string> compilerParts = {
      "Code Generation",
      "Handles code generation",
      "Supports execution of generated code",
      "Handles machine code generation",
      "Deals with object file manipulation",
      "Targets specific architectures",
      "Assembly and Bitcode",
      "Parses assembly files",
      "Handles LLVM bitcode",
      "Support",
      "Provides various utility functions",
      "Analysis",
      "Static analysis in Clang",
      "General LLVM analysis passes",
      "IR Manipulation",
      "Core LLVM Intermediate Representation (IR) manipulation",
      "Reads LLVM IR",
      "Object and Binary Format",
      "Deals with binary formats",
      "Passes and Transformations",
      "Collection of optimization passes",
      "Framework for LLVM transformations",
      "Frontend",
      "Header files for Clang frontend",
      "Handles Abstract Syntax Tree (AST)",
      "Implements Clang frontend",
      "Support Libraries",
      "Backend",
      "Middle-End",
      "Frontend",
      "StaticAnalyzer",
      "Sema",
      "Serialization",
      "Parse",
      "Lex",
      "AST"};
  std::vector<std::string> programmingLanguage = {
      "Integer",
      "Floating-Point Numbers",
      "ASCII Value of a Character",
      "Print",
      "Compute Quotient and Remainder",
      "int, float, double and char",
      "sizeof",
      "Keyword long",
      "Swap Two Numbers",
      "Number is Even or Odd",
      "A character is a Vowel or Consonant",
      "Largest Number Among Three Numbers",
      "binary operators",
      "short circuiting rule",
      "short circuit",
      "Roots of a Quadratic Equation",
      "logic expression",
      "expression",
      "expression not zero",
      "expression positive",
      "expression negative",
      "expression negative or positive",
      "Leap Year",
      "a Number is Positive or Negative",
      "a Character is an Alphabet or not",
      "the Sum of Natural Numbers",
      "calculate two numbers result",
      "Factorial of a Number",
      "Generate Multiplication Table",
      "Generate",
      "Fibonacci Sequence",
      "GCD of two Numbers",
      "LCM of two Numbers",
      "Characters from A to Z Using Loop",
      "Loop",
      "For loop",
      "Case switch",
      "Count Number of Digits in an Integer",
      "Reverse a Number",
      "Calculate the Power of a Number",
      "a Number is Palindrome or Not",
      "Number is Prime or Not",
      "Prime Numbers Between Two Intervals",
      "Armstrong Number",
      "Armstrong Number Between Two Intervals",
      "Factors of a Number",
      "Simple Calculator Using switch...case",
      "Prime Numbers Between Intervals Using Function",
      "Prime or Armstrong Number Using User-defined Function",
      "User-defined Function",
      "Macros",
      "function",
      "foo function",
      "Recursion",
      "logical",
      "logic",
      "time in AM-PM format",
      "Digital Clock with the Current Time",
      "Calendar Year in Different Formats",
      "Dates",
      "Time",
      "Structure in C",
      "Structures and Sort them by Age or ID",
      "Passing Structure to a Function",
      "Complex numbers",
      "N Distances Given in inch-feet System using Structures",
      "Structures and Sort them by Name",
      "Students Using Structure",
      "Union",
      "int32",
      "Singly Linked List using Recursion",
      "List",
      "array",
      "heap",
      "Array of Strings",
      "regex",
      "null",
      "null pointer check",
      "check before dividing",
      "Number of Sub-Strings ",
      "Array Rotation ",
      "Two Arrays",
      "merge",
      "element in array",
      "min",
      "max",
      "const",
      "C Program to Sort an Array using Bubble Sort",
      "C Program to Sort an Array using Merge Sort",
      "C Program to Sort an Array Using Selection Sort",
      "C Program to Sort an Array Using Insertion Sort",
      "C Program For Boolean to String Conversion",
      "C Program For Float to String Conversion",
      "C Program For Double to String Conversion",
      "C Program For String to Long Conversion",
      "C Program For Long to String Conversion",
      "C Program For Int to Char Conversion",
      "C Program For Char to Int Conversion",
      "C Program For Octal to Decimal Conversion",
      "C Program For Decimal to Octal Conversion",
      "C Program For Hexadecimal to Decimal Conversion",
      "C Program For Decimal to Hexadecimal Conversion",
      "C Program For Decimal to Binary Conversion",
      "C Program For Binary to Decimal Conversion",
      "C Program to Print an Integer (Entered by the User)",
      "C Program to Add Two Integers",
      "C Program to Multiply Two Floating-Point Numbers",
      "C Program to Find ASCII Value of a Character",
      "C Program to Compute Quotient and Remainder",
      "C Program to Find the Size of int, float, double and char",
      "C Program to Demonstrate the Working of Keyword long",
      "C Program to Swap Two Numbers",
      "C Program to Check Whether a Number is Even or Odd",
      "C Program to Check Whether a Character is a Vowel or Consonant",
      "C Program to Find the Largest Number Among Three Numbers",
      "C Program to Find the Roots of a Quadratic Equation",
      "C Program to Check Leap Year",
      "C Program to Check Whether a Number is Positive or Negative",
      "C Program to Check Whether a Character is an Alphabet or not",
      "C Program to Calculate the Sum of Natural Numbers",
      "C Program to Find Factorial of a Number",
      "C Program to Generate Multiplication Table",
      "C Program to Display Fibonacci Sequence",
      "C Program to Find GCD of two Numbers",
      "C Program to Find LCM of two Numbers",
      "C Program to Display Characters from A to Z Using Loop",
      "C Program to Count Number of Digits in an Integer",
      "C Program to Reverse a Number",
      "C Program to Calculate the Power of a Number",
      "C Program to Check Whether a Number is Palindrome or Not",
      "C Program to Check Whether a Number is Prime or Not",
      "C Program to Display Prime Numbers Between Two Intervals",
      "C Program to Check Armstrong Number",
      "C Program to Display Armstrong Number Between Two Intervals",
      "C Program to Display Factors of a Number",
      "C Program to Make a Simple Calculator Using switch...case",
      "C Program to Display Prime Numbers Between Intervals Using Function",
      "C Program to Check Prime or Armstrong Number Using User-defined "
      "Function",
      "C Program to Check Whether a Number can be Expressed as Sum of Two "
      "Prime Numbers",
      "C Program to Find the Sum of Natural Numbers using Recursion",
      "C Program to Find Factorial of a Number Using Recursion",
      "C Program to Find G.C.D Using Recursion",
      "C Program to Convert Binary Number to Decimal and vice-versa",
      "C Program to Convert Octal Number to Decimal and vice-versa",
      "C Program to Convert Binary Number to Octal and vice-versa",
      "C Program to Reverse a Sentence Using Recursion",
      "C program to calculate the power using recursion",
      "C Program to Calculate Average Using Arrays",
      "C Program to Find Largest Element in an Array",
      "C Program to Calculate Standard Deviation",
      "C Program to Add Two Matrices Using Multi-dimensional Arrays",
      "C Program to Multiply Two Matrices Using Multi-dimensional Arrays",
      "C Program to Find Transpose of a Matrix",
      "C Program to Multiply two Matrices by Passing Matrix to a Function",
      "C Program to Access Array Elements Using Pointer",
      "C Program Swap Numbers in Cyclic Order Using Call by Reference",
      "C Program to Find Largest Number Using Dynamic Memory Allocation",
      "C Program to Find the Frequency of Characters in a String",
      "C Program to Count the Number of Vowels, Consonants and so on",
      "C Program to Remove all Characters in a String Except Alphabets",
      "C Program to Find the Length of a String",
      "C Program to Concatenate Two Strings",
      "C Program to Copy String Without Using strcpy()",
      "C Program to Sort Elements in Lexicographical Order (Dictionary Order)",
      "C Program to Store Information of a Student Using Structure",
      "C Program to Add Two Distances (in inch-feet system) using Structures",
      "C Program to Add Two Complex Numbers by Passing Structure to a Function",
      "C Program to Calculate Difference Between Two Time Periods",
      "C Program to Store Information of Students Using Structure",
      "C Program to Store Data in Structures Dynamically",
      "C Program to Write a Sentence to a File",
      "C Program to Read the First Line From a File",
      "C Program to Display its own Source Code as Output",
      "C Program to Print Pyramids and Patterns",
      "Short circuit and operator precedence",
      "using scalars with short-circuiting or",
      "Order of evaluation between logical and assignment operators",
      "a function call",
      "input include several arguments",
      "Checking dictionary key existence and boolean condition",
      "Short Circuit Evaluation",
      "Something cool!",
      "Anything popular in C",
      "recursion",
      "short circuiting operations",
      "conditions of short circuit",
      "use a value without computing it twice",
      "Ternary operator",
      "Ternary operator use with short circuiting operations",
      "Character sets",
      "Character display semantics",
      "limits of integers",
      "Types",
      "while",
      "For and While loops",
      "size_t",
      "Complex numbers",
      "malloc",
      "EOF",
      "STDC",
      "struct, union or enum, tv_sec",
      "sqrt",
      "! (exclamation-mark punctuator),",
      "! (logical negation operator)",
      "!= (inequality operator)",
      "!= (not-equal punctuator)",
      "# (hash punctuator)",
      "# preprocessing directive",
      "# punctuator",
      "#define preprocessing directive",
      "#elif preprocessing directive",
      "#else preprocessing directive",
      "#endif preprocessing directive",
      "#error preprocessing directive",
      "#if preprocessing directive",
      "#if preprocessing directive",
      "#ifdef preprocessing directive",
      "#ifndef preprocessing directive",
      "#include preprocessing directive",
      "#line preprocessing directive",
      "#pragma preprocessing directive",
      "#undef preprocessing directive",
      "& (bitwise AND operator)",
      "&& (logical AND operator)",
      "&= (bitwise AND assignment operator)",
      "’ ’ (space character)",
      "( (opening parenthesis punctuator)",
      "() (cast operator)",
      "() (function-call operator)",
      "() (parentheses punctuator)",
      "(){} (compound-literal operator)",
      ") (closing parenthesis punctuator)",
      "* (asterisk punctuator)",
      "* (indirection operator)",
      "* (multiplication operator)",
      "*= (asterisk-equal punctuator)",
      "*= (multiplication assignment operator)",
      "+ (addition operator)",
      "+ (plus punctuator)",
      "+ (unary plus operator)",
      "++ (plus-plus punctuator)",
      "++ (postfix increment operator)",
      "++ (prefix increment operator)",
      "+= (addition assignment operator)",
      "+= (plus-equal punctuator)",
      ", (comma operator)",
      ", (comma punctuator)",
      "- (minus punctuator)",
      "- (subtraction operator)",
      "- (unary minus operator)",
      "- format flag",
      "-- (minus-minus punctuator)",
      "-- (postfix decrement operator)",
      "-- (prefix decrement operator)",
      "-= (minus-equal punctuator)",
      "-= (subtraction assignment operator)",
      "-> (minus-greater punctuator)",
      "-> (structure/union pointer operator)",
      ". (dot punctuator)",
      ". (structure/union member operator)",
      "... (ellipsis punctuator)",
      "/ (division operator)",
      "/ (slash punctuator)",
      "/= (division assignment operator)",
      "/= (slash-equal punctuator)",
      ": (colon punctuator)",
      ":> (colon greater punctuator)",
      "; (semicolon punctuator)",
      "< (less punctuator)",
      "< (less-than operator)",
      "<: (alternative spelling of [)",
      "<: (less-colon punctuator)",
      "<< (left-shift operator)",
      "<< (less-less punctuator)",
      "<<= (left-shift assignment operator)",
      "<<= (less-less equal punctuator)",
      "<= (less-equal punctuator)",
      "<= (less-than-or-equal-to operator)",
      "<% (alternative spelling of {)",
      "<% (less-percent punctuator)",
      "<assert.h> header",
      "<complex.h> header",
      "<ctype.h> header",
      "<errno.h> header",
      "<fenv.h> header",
      "<float.h> header",
      "<inttypes.h> header",
      "<iso646.h> header",
      "<limits.h> header",
      "<locale.h> header",
      "<math.h>",
      "<setjmp.h> header,",
      "<signal.h> header,",
      "<stdalign.h> header,",
      "<stdarg.h> header,",
      "<stdatomic.h> header,",
      "<stdbool.h> header,",
      "<stddef.h> header,",
      "<stdint.h> header,",
      "<stdio.h> header,",
      "<stdlib.h> header,",
      "<stdnoreturn.h> header,",
      "<string.h> header,",
      "<tgmath.h> header,",
      "<threads.h> header,",
      "<time.h> header,",
      "<uchar.h> header,",
      "<wchar.h> header,",
      "<wctype.h> header,",
      "= (equal-sign punctuator),",
      "= (simple assignment operator),",
      "== (equal-equal punctuator),",
      "== (equality operator),",
      "> (greater punctuator),",
      "> (greater-than operator),",
      ">= (greater-equal punctuator),",
      ">= (greater-than-or-equal-to operator),",
      ">> (greater greater punctuator),",
      ">> (right-shift operator),",
      ">>= (greater-greater-equal punctuator),",
      ">>= (right-shift assignment operator),",
      "? (question-mark punctuator),",
      "?: (conditional operator),",
      "?? (trigraph sequences),",
      "[ (opening bracket punctuator),",
      "[ ] (array subscript operator),",
      "[ ] (brackets punctuator),",
      "# format flag,",
      "% (percent punctuator),",
      "% (remainder operator),",
      "%: (percent-colon punctuator),",
      "%:%: (percent-percent punctuator),",
      "%= (percent-equal punctuator),",
      "%= (remainder assignment operator),",
      "%> (alternative spelling of }),",
      "%> (percent-greater punctuator),",
      "%A conversion specifier,",
      "%B conversion specifier,",
      "%C conversion specifier,",
      "%D conversion specifier,",
      "%E conversion specifier,",
      "%F conversion specifier,",
      "%G conversion specifier,",
      "%H conversion specifier,",
      "%I conversion specifier,",
      "%M conversion specifier,",
      "%R conversion specifier,",
      "%S conversion specifier,",
      "%T conversion specifier,",
      "%U conversion specifier,",
      "%V conversion specifier,",
      "%W conversion specifier,",
      "%X conversion specifier,",
      "%Y conversion specifier,",
      "%Z conversion specifier,",
      "& (address operator),",
      "& (ampersand punctuator),",
      "&= (ampersand-equal punctuator),",
      "&& (ampersand-ampersand punctuator),",
      "(backslash character),",
      "(backslash escape sequence),",
      "(single-quote escape sequence)",
      "(null character)",
      "padding of binary stream,",
      "(question-mark escape sequence)",
      " (universal character names)",
      "(alert escape sequence)",
      " (backspace escape sequence)",
      "(escape character)",
      "(form-feed escape sequence)",
      "(new-line escape sequence)",
      "octal digits (octal-character escape sequence),",
      "(carriage-return escape sequence)",
      "(horizontal-tab escape sequence)",
      "(universal character names),",
      "(vertical-tab escape sequence),",
      "{} (braces punctuator),",
      "{ (opening brace punctuator),",
      "} (closing brace punctuator),",
      "] (closing bracket punctuator),",
      "^ (bitwise exclusive OR operator),",
      "^ (caret punctuator),",
      "^= (bitwise exclusive OR assignment operator),",
      "^= (caret-equal punctuator),",
      "_Alignas,",
      "_Alignof operator,",
      "_Atomic type qualifier,",
      "_Atomic type specifier,",
      "_Bool type,",
      "_Bool type conversions,",
      "_C identifier suffix,",
      "_Complex type,",
      "_Complex types,",
      "_Complex_I macro,",
      "_Exit function,",
      "_Generic,",
      "_IOFBF macro,",
      "_IOLBF macro,",
      "_IONBF macro,",
      "_Imaginary keyword,",
      "_Imaginary type,",
      "_Imaginary types,",
      "_Imaginary_I macro,",
      "_MAX identifier suffix,",
      "_MIN identifier suffix,",
      "_Noreturn,",
      "_Pragma operator,",
      "_Static_assert,",
      "_Thread_local storage-class specifier,",
      "__STDC_ identifier prefix,",
      "__DATE__ macro,",
      "__FILE__ macro,",
      "__LINE__ macro,",
      "__STDC_ANALYZABLE__ macro,",
      "__STDC_HOSTED__ macro,",
      "__STDC_IEC_559_COMPLEX__ macro,",
      "__STDC_IEC_559__ macro,",
      "__STDC_ISO_10646__ macro,",
      "__STDC_LIB_EXT1__ macro,",
      "__STDC_MB_MIGHT_NEQ_WC__ macro,",
      "__STDC_NO_ATOMICS__ macro,",
      "__STDC_NO_COMPLEX__ macro,",
      "__STDC_NO_THREADS__ macro,",
      "__STDC_NO_VLA__ macro,",
      "__STDC_UTF_16__ macro,",
      "__STDC_UTF_32__ macro,",
      "__STDC_VERSION__ macro,",
      "__STDC_WANT_LIB_EXT1__ macro,",
      "__STDC__ macro,",
      "__TIME__ macro,",
      "__VA_ARGS__ identifier,",
      "__alignas_is_defined macro,",
      "__alignof_is_defined macro,",
      "__bool_true_false_are_defined macro,",
      "__cplusplus macro,",
      "__func__ identifier,",
      "_explicit identifier suffix,",
      "_t identifier suffix,",
      "| (bitwise inclusive OR operator),",
      "| (vertical-line punctuator),",
      "|= (bitwise inclusive OR assignment operator),",
      "|= (vertical-line-equal punctuator),",
      "|| (logical OR operator),",
      "|| (vertical-vertical punctuator),",
      "~ (bitwise complement operator),",
      "~ (tilde punctuator),",
      "0 format flag,",
      "abort function,",
      "abort_handler_s function,",
      "abs function,",
      "abs macro,",
      "absolute-value functions",
      "complex,",
      "integer,",
      "real,",
      "abstract declarator,",
      "abstract machine,",
      "access",
      "accuracy, see floating-point accuracy",
      "acos function",
      "acos type-generic macro",
      "acosf function",
      "acosh function",
      "acosh type-generic macro",
      "acoshf function",
      "acoshl function",
      "acosl function",
      "acquire fence",
      "acquire operation",
      "active position",
      "addition assignment operator (+=)",
      "addition operator (+)",
      "additive expressions",
      "address constant",
      "address operator (&)",
      "address-free",
      "aggregate initialization",
      "aggregate types",
      "alert",
      "alert escape sequence (a)",
      "aliasing",
      "alignas macro",
      "aligned_alloc function",
      "alignment",
      "pointer",
      "structure/union member",
      "alignment header",
      "alignment specifier",
      "alignof macro",
      "allocated storage, order and contiguity",
      "alternative spellings header",
      "and macro",
      "AND operators",
      "bitwise (&)",
      "bitwise (&)",
      "bitwise assignment",
      "logical (&&)",
      "AND operators",
      "logical (&&)",
      "and_eq macro",
      "anonymous structure",
      "anonymous union",
      "ANSIEE 754",
      "ANSIEE 854",
      "argc (main function parameter)",
      "argument",
      "array",
      "default promotions",
      "function",
      "macro",
      "argument, complex",
      "argv (main function parameter)",
      "arithmetic constant expression",
      "arithmetic conversions",
      "arithmetic operators",
      "additive",
      "bitwise",
      "increment and decrement",
      "multiplicative",
      "shift",
      "unary",
      "arithmetic types",
      "arithmetic, pointer",
      "array",
      "argument",
      "declarator",
      "initialization",
      "multidimensional",
      "parameter",
      "storage order",
      "subscript operator ([ ])",
      "subscripting",
      "type",
      "type conversion",
      "variable length",
      "arrow operator (->)",
      "as–if rule",
      "ASCII code set",
      "asctime function",
      "asctime_s function",
      "asin function",
      "asin type-generic macro",
      "asinf function",
      "asinh function",
      "asinh type-generic macro",
      "asinhf function",
      "asinhl function",
      "asinl function",
      "assert macro",
      "assignment",
      "compound",
      "conversion",
      "expression",
      "operators",
      "simple",
      "associativity of operators",
      "asterisk punctuator (*)",
      "at_quick_exit function",
      "atan function",
      "atan type-generic macro",
      "atan2 function",
      "atan2 type-generic macro",
      "atan2f function",
      "atan2l function",
      "atanf function",
      "atanh function",
      "atanh type-generic macro",
      "atanhf function",
      "atanhl function",
      "atanl function",
      "atexit function",
      "atof function",
      "atoi function",
      "atol function",
      "atoll function",
      "atomic lock-free macros",
      "atomic operations",
      "atomic types",
      "ATOMIC_ identifier prefix",
      "atomic_ identifier prefix I",
      "atomic_bool type",
      "ATOMIC_BOOL_LOCK_FREE macro",
      "atomic_char type",
      "atomic_char16_t type",
      "ATOMIC_CHAR16_T_LOCK_FREE macro",
      "atomic_char32_t type",
      "ATOMIC_CHAR32_T_LOCK_FREE macro",
      "ATOMIC_CHAR_LOCK_FREE macro",
      "atomic_compare_exchange_strong function",
      "atomic_compare_exchange_strong_explicit function",
      "atomic_compare_exchange_weak function",
      "atomic_compare_exchange_weak_explicit function",
      "atomic_exchange function",
      "atomic_exchange_explicit function",
      "atomic_fetch_ function",
      "atomic_flag type",
      "atomic_flag_clear function",
      "atomic_flag_clear_explicit function",
      "ATOMIC_FLAG_INIT macro",
      "atomic_flag_test_and_set function",
      "atomic_flag_test_and_set_explicit function",
      "atomic_init function",
      "atomic_int type",
      "atomic_int_fast16_t type",
      "atomic_int_fast32_t type",
      "atomic_int_fast64_t type",
      "atomic_int_fast8_t type",
      "atomic_int_least16_t type",
      "atomic_int_fast32_t type",
      "atomic_int_fast64_t type",
      "atomic_int_fast8_t type",
      "atomic_int_least16_t type",
      "atomic_int_least32_t type",
      "atomic_int_least64_t type",
      "atomic_int_least8_t type",
      "ATOMIC_INT_LOCK_FREE macro",
      "atomic_intmax_t type",
      "atomic_intptr_t type",
      "atomic_is_lock_free function",
      "atomic_llong type",
      "ATOMIC_LLONG_LOCK_FREE macro",
      "atomic_load function",
      "atomic_load_explicit function",
      "atomic_long type",
      "ATOMIC_LONG_LOCK_FREE macro",
      "ATOMIC_POINTER_LOCK_FREE macro",
      "atomic_ptrdiff_t type",
      "atomic_schar type",
      "atomic_short type",
      "ATOMIC_SHORT_LOCK_FREE macro",
      "atomic_signal_fence function",
      "atomic_size_t type",
      "atomic_store function",
      "atomic_store_explicit function",
      "atomic_thread_fence function",
      "atomic_uchar type",
      "atomic_uint type",
      "atomic_uint_fast16_t type",
      "atomic_uint_fast32_t type",
      "atomic_uint_fast64_t type",
      "atomic_uint_fast8_t type",
      "atomic_uint_least16_t type",
      "atomic_uint_least32_t type",
      "atomic_uint_least64_t type",
      "atomic_uint_least8_t type",
      "atomic_uintmax_t type",
      "atomic_uintptr_t type",
      "atomic_ullong type",
      "atomic_ulong type",
      "atomic_ushort type",
      "ATOMIC_VAR_INIT (obsolete)",
      "atomic_wchar_t type",
      "ATOMIC_WCHAR_T_LOCK_FREE macro",
      "atomics header",
      "auto storage-class specifier",
      "automatic storage duration",
      "backslash character",
      "backslash escape sequence",
      "backspace",
      "backspace escape sequence",
      "basic character set",
      "basic types",
      "behavior",
      "binary streams",
      "bit",
      "high order",
      "low order",
      "bit-field",
      "bitand macro",
      "bitor macro",
      "bitwise operators",
      "AND",
      "AND assignment (&=)",
      "complement (~)",
      "exclusive OR",
      "exclusive OR assignment (^=)",
      "inclusive OR",
      "inclusive OR assignment (|=)",
      "shift",
      "blank character",
      "block",
      "block scope",
      "block structure",
      "bold type convention",
      "boolean type",
      "boolean type and values header",
      "boolean type conversion",
      "bounded undefined behavior",
      "braces punctuator ({})",
      "brackets operator ([])",
      "brackets punctuator ([ ])",
      "branch cuts",
      "broken-down time",
      "bsearch function",
      "bsearch_s function",
      "btowc function",
      "BUFSIZ macro",
      "byte",
      "byte input/output functions",
      "byte-oriented stream",
      "C program",
      "c16rtomb function",
      "c32rtomb function",
      "cabs function",
      "cabs functions",
      "type-generic macro for",
      "cabsf function",
      "cabsl function",
      "cacos function",
      "cacos functions",
      "cacosf function",
      "cacosh function",
      "cacosh functions",
      "cacoshf function",
      "cacoshl function",
      "cacosl function",
      "calendar time",
      "call by value",
      "call_once function",
      "calloc function",
      "carg function",
      "carg functions",
      "carg type-generic macro",
      "cargf function",
      "cargl function",
      "carriage return",
      "carriage-return escape sequence (\r)",
      "carries a dependency",
      "case label",
      "case mapping functions",
      "character",
      "wide character",
      "extensible",
      "casin function",
      "casin functions",
      "casinf function",
      "casinh function",
      "casinh functions",
      "casinhf function",
      "casinhl function",
      "casinl function",
      "cast",
      "cast expression",
      "cast operator (())",
      "catan function",
      "catan functions",
      "catanf function",
      "catanh function",
      "catanh functions",
      "catanhf function",
      "catanhl function",
      "catanl function",
      "cbrt function",
      "cbrt type-generic macro",
      "cbrtf function",
      "cbrtl function",
      "ccos function",
      "ccos functions",
      "ccosf function",
      "ccosh function",
      "ccosh functions",
      "ccoshf function",
      "ccoshl function",
      "ccosl function",
      "ceil function",
      "ceil type-generic macro",
      "ceilf function",
      "ceill function",
      "cerf function",
      "cerfc function",
      "cexp function",
      "cexp functions",
      "cexp2 function",
      "cexpf function",
      "cexpl function",
      "cexpm1 function",
      "char type",
      "char type conversion",
      "char16_t type",
      "char32_t type",
      "CHAR_BIT macro",
      "CHAR_MAX macro",
      "CHAR_MIN macro",
      "character array initialization",
      "character case mapping functions",
      "character classification functions",
      "character constant",
      "character display semantics",
      "character handling header",
      "character input/output functions",
      "character sets",
      "character string literal, see string literal",
      "character type conversion",
      "character types",
      "characteristics of floating types header",
      "cimag function",
      "cimag functions",
      "cimag type-generic macro",
      "cimagf function",
      "cimagl function",
      "cis function",
      "classification functions",
      "floating-point",
      "clearerr function",
      "clgamma function",
      "clock function",
      "clock_t type",
      "CLOCKS_PER_SEC macro",
      "clog function",
      "clog functions",
      "clog10 function",
      "clog1p function",
      "clog2 function",
      "clogf function",
      "clogl function",
      "closing",
      "CMPLX macro",
      "CMPLXF macro",
      "CMPLXL macro",
      "cnd_ identifier prefix",
      "cnd_broadcast function",
      "cnd_destroy function",
      "cnd_init function",
      "cnd_signal function",
      "cnd_t type",
      "cnd_timedwait function",
      "cnd_wait function",
      "collating sequences",
      "colon punctuator (:)",
      "comma operator (,)",
      "comma punctuator (,)",
      "command processor",
      "comment delimiters (/* */ and //)",
      "comments",
      "common definitions header",
      "common extensions",
      "common initial sequence",
      "common real type",
      "common warnings",
      "comparison functions",
      "string",
      "wide string",
      "comparison macros",
      "comparison, pointer",
      "compatible type",
      "compl macro",
      "complement operator (~)",
      "complete",
      "complete type",
      "complex arithmetic header",
      "complex macro",
      "complex numbers",
      "complex type conversion",
      "complex type domain",
      "complex types",
      "compliance, see conformance",
      "components of time",
      "composite type",
      "compound assignment",
      "compound literal",
      "compound literals",
      "compound statement",
      "compound-literal operator",
      "concatenation functions",
      "concatenation, preprocessing, see preprocessing",
      "concatenation",
      "conceptual models",
      "conditional features",
      "conditional inclusion",
      "conditional operator (?:)",
      "conflict",
      "conformance",
      "conforming freestanding implementation",
      "conforming hosted implementation",
      "conforming implementation",
      "conforming program",
      "conj function",
      "conj functions",
      "conj type-generic macro",
      "conjf function",
      "conjl function",
      "const type qualifier",
      "const-qualified type",
      "constant expression",
      "constants",
      "as primary expression",
      "enumeration",
      "floating",
      "hexadecimal",
      "integer",
      "octal",
      "constraint",
      "constraint_handler_t type",
      "consume operation",
      "content of structure/union/enumeration",
      "contiguity of allocated storage",
      "continue",
      "contracted",
      "contracted expression",
      "control character",
      "control wide character",
      "conversion",
      "arithmetic operands",
      "array argument",
      "array parameter",
      "arrays",
      "boolean",
      "boolean, characters, and integers",
      "by assignment",
      "by return statement",
      "explicit",
      "function",
      "function argument",
      "function designators",
      "function parameter",
      "imaginary",
      "imaginary and complex",
      "implicit",
      "lvalues",
      "pointer",
      "real and complex",
      "real and imaginary",
      "real floating and integer",
      "real floating types",
      "signed and unsigned integers",
      "usual arithmetic, see usual arithmetic conversions",
      "void type",
      "conversion functions",
      "multibyte/wide character",
      "extended",
      "restartable",
      "multibyte/wide string",
      "numeric",
      "single byte/wide character",
      "time",
      "conversion specifier",
      "conversion state",
      "conversion state functions",
      "copying functions",
      "copysign function",
      "copysign type-generic macro",
      "copysignf function",
      "copysignl function",
      "correctly rounded result",
      "corresponding real type",
      "cos function",
      "cos type-generic macro",
      "cosf function",
      "cosh function",
      "cosh type-generic macro",
      "coshf function",
      "coshl function",
      "cosl function",
      "cpow function",
      "cpow functions",
      "cpowf function",
      "cpowl function",
      "cproj function",
      "cproj functions",
      "cproj type-generic macro",
      "cprojf function",
      "cprojl function",
      "creal function",
      "creal functions",
      "creal type-generic macro",
      "crealf function",
      "creall function",
      "creating",
      "critical undefined behavior",
      "csin function",
      "csin functions",
      "csinf function",
      "csinh function",
      "csinh functions",
      "csinhf function",
      "csinhl function",
      "csinl function",
      "csqrt function",
      "csqrt functions",
      "csqrtf function",
      "csqrtl function",
      "ctan function",
      "ctan functions",
      "ctanf function",
      "ctanh function",
      "ctanh functions",
      "ctanhf function",
      "ctanhl function",
      "ctanl function",
      "ctgamma function",
      "ctime function",
      "ctime_s function",
      "currency_symbol structure member",
      "current object",
      "CX_LIMITED_RANGE pragma",
      "data race",
      "data stream",
      "date and time header",
      "Daylight Saving Time",
      "DBL_DECIMAL_DIG macro",
      "DBL_DIG macro",
      "DBL_EPSILON macro",
      "DBL_HAS_SUBNORM macro",
      "DBL_MANT_DIG macro",
      "DBL_MAX macro",
      "DBL_MAX_10_EXP macro",
      "DBL_MAX_EXP macro",
      "DBL_MIN macro",
      "DBL_MIN_10_EXP macro",
      "DBL_MIN_EXP macro",
      "DBL_TRUE_MIN macro",
      "decimal constant",
      "decimal digit",
      "decimal-point character",
      "DECIMAL_DIG macro",
      "decimal_point structure member",
      "declaration specifiers",
      "declarations",
      "structure/union",
      "typedef",
      "declarator",
      "abstract",
      "declarator type derivation",
      "decrement operators, see arithmetic operators, increment and decrement",
      "default argument promotions",
      "default initialization",
      "default label",
      "DEFAULT pragma, defined operator",
      "definition, function",
      "dependency-ordered before",
      "dereference, see indirection operator",
      "derived declarator types",
      "derived types",
      "designated initializer",
      "destringizing",
      "device input/output",
      "diagnostic message",
      "diagnostics",
      "diagnostics header",
      "difftime function",
      "digit",
      "digits",
      "digraphs",
      "direct input/output functions",
      "display device",
      "div function, div_t type",
      "division assignment operator (/=)",
      "division operator (/)",
      "documentation of implementation",
      "domain error, dot operator (.)",
      "double _Complex type",
      "double _Complex type conversion",
      "double _Imaginary type",
      "double type",
      "double type conversion",
      "double-precision arithmetic",
      "double-quote escape sequence (\")",
      "double_t type",
      "E format modifier",
      "E identifier prefix",
      "EDOM macro, effective type",
      "EILSEQ macro",
      "element type",
      "ellipsis punctuator (...)",
      "else statement",
      "empty statement",
      "encoding error",
      "end-of-file, end-of-file indicator",
      "end-of-file macro, see EOF macro",
      "end-of-line indicator",
      "enum type",
      "enumerated type",
      "enumeration constant",
      "enumeration content",
      "enumeration members",
      "enumeration specifiers",
      "enumeration tag",
      "enumerator",
      "environment",
      "environment functions",
      "environment list",
      "environmental considerations",
      "environmental limits",
      "EOF macro, epoch",
      "equal-sign punctuator (=)",
      "equal-to operator, see equality operator",
      "equality expressions",
      "equality operator (==)",
      "ERANGE macro, erf function, erf type-generic macro, erfc function, erfc "
      "type-generic macro, erfcf function, erfcl function, erff function, erfl "
      "function",
      "errno identifier",
      "errno_t type",
      "error conditions",
      "error functions",
      "error indicator",
      "error preprocessing directive",
      "error-handling functions",
      "errors header",
      "escape character",
      "escape sequences",
      "evaluation format",
      "evaluation method",
      "evaluation of expression",
      "evaluation order, see order of evaluation",
      "exceptional condition",
      "excess precision",
      "excess range",
      "exclusive OR operators",
      "bitwise (^), bitwise assignment (^=)",
      "executable program",
      "execution character set",
      "execution environment, see also environmental limits",
      "execution sequence",
      "EXIT_FAILURE macro, EXIT_SUCCESS macro",
      "exp function",
      "explicit conversion",
      "expm1 function, expm1 type-generic macro, expm1f function, expm1l "
      "function",
      "exponent part",
      "exponential functions",
      "complex, real",
      "expression",
      "assignment, cast, constant, evaluation, full, order of evaluation, "
      "parenthesized, primary, unary, void",
      "expression statement",
      "extended alignment",
      "extended character set",
      "extended characters",
      "extended integer types",
      "extended multibyte and wide character utilities header",
      "extended signed integer types",
      "extended unsigned integer types",
      "extensible wide character case mapping functions",
      "extensible wide character classification functions",
      "extern storage-class specifier",
      "external definition",
      "external identifiers, underscore",
      "external linkage",
      "external name",
      "external object definitions",
      "f identifier suffix",
      "fabs function, fabs type-generic macro, fabsf function, fabsl function",
      "false macro",
      "fclose function",
      "fdim function",
      "FE_ identifier prefix, FE_ALL_EXCEPT macro, FE_DFL_ENV macro, "
      "FE_DIVBYZERO macro, FE_DOWNWARD macro, FE_INEXACT macro, FE_INVALID "
      "macro, FE_OVERFLOW macro, FE_TONEAREST macro, FE_TOWARDZERO macro, "
      "FE_UNDERFLOW macro, FE_UPWARD macro",
      "feclearexcept function",
      "fegetenv function",
      "fegetexceptflag function",
      "fegetround function",
      "feholdexcept function",
      "fence",
      "fences",
      "exp type-generic macro",
      "exp2 function",
      "exp2 type-generic macro",
      "exp2f function",
      "exp2l function",
      "expf function",
      "expl function",
      "fdim type-generic macro",
      "fdimf function",
      "fdiml function",
      "extended multibyte/wide character conversion utilities",
      "FENV_ACCESS pragma",
      "fenv_t type",
      "feof function",
      "feraiseexcept function",
      "ferror function",
      "fesetenv function",
      "fesetexceptflag function",
      "fesetround function",
      "fetestexcept function",
      "feupdateenv function",
      "fexcept_t type",
      "fflush function",
      "fgetc function",
      "fgetpos function",
      "fgets function",
      "fgetwc function",
      "fgetws function",
      "field width",
      "file",
      "access functions, name",
      "operations, position indicator, positioning functions",
      "file name",
      "file position indicator",
      "file scope",
      "FILE type",
      "FILENAME_MAX macro",
      "finite number",
      "flags",
      "flexible array member",
      "float _Complex type",
      "float _Complex type conversion",
      "float _Imaginary type",
      "float type",
      "float type conversion",
      "float_t type",
      "floating constant",
      "floating suffix, f or F",
      "floating type conversion",
      "floating types",
      "floating-point accuracy",
      "floating-point arithmetic functions",
      "floating-point classification functions",
      "floating-point control mode",
      "floating-point environment",
      "floating-point environment header",
      "floating-point exception",
      "floating-point number",
      "floating-point rounding mode",
      "floating-point status flag",
      "floor function, floor type-generic macro",
      "floorf function",
      "floorl function",
      "FLT_DECIMAL_DIG macro",
      "FLT_DIG macro",
      "FLT_EPSILON macro",
      "FLT_EVAL_METHOD macro",
      "FLT_HAS_SUBNORM macro",
      "FLT_MANT_DIG macro",
      "FLT_MAX macro",
      "FLT_MAX_10_EXP macro",
      "FLT_MAX_EXP macro",
      "FLT_MIN macro",
      "FLT_MIN_10_EXP macro",
      "FLT_MIN_EXP macro",
      "FLT_RADIX macro",
      "FLT_ROUNDS macro",
      "FLT_TRUE_MIN macro",
      "fma function, fma type-generic macro, fmaf function, fmal function",
      "fmax function, fmax type-generic macro, fmaxf function, fmaxl function",
      "fmin function, fmin type-generic macro, fminf function, fminl function",
      "fmod function, fmod type-generic macro, fmodf function, fmodl function",
      "fopen function, FOPEN_MAX macro, fopen_s function",
      "for statement",
      "form feed",
      "form-feed character",
      "form-feed escape sequence (\f)",
      "format conversion of integer types header",
      "format flag",
      "format modifier",
      "formatted input/output functions, wide character",
      "forward reference",
      "FP_ identifier prefix",
      "FP_CONTRACT pragma, FP_FAST_FMA macro, FP_FAST_FMAF macro",
      "fpos_t type",
      "fprintf_s function",
      "fputc function",
      "fputs function",
      "fputwc function",
      "fputws function",
      "frac_digits structure member",
      "fread function",
      "free function",
      "freestanding",
      "freestanding execution environment",
      "freopen function",
      "freopen_s function",
      "frexp function, frexp type-generic macro, frexpf function, frexpl "
      "function",
      "fscanf function, fscanf_s function",
      "fseek function",
      "fsetpos function",
      "ftell function",
      "full declarator",
      "full expression",
      "fully buffered",
      "fully buffered stream",
      "argument, body, call, declarator, definition, designator, image",
      "function prototype scope",
      "function scope",
      "function specifiers",
      "function type",
      "function-call operator (())",
      "function-like macro",
      "fundamental alignment",
      "future directions",
      "language, library",
      "fwide function",
      "fwprintf function, fwprintf_s function",
      "fwrite function",
      "fwscanf function, fwscanf_s function",
      "gamma functions",
      "general utilities",
      "FP_FAST_FMAL macro, FP_ILOGB0 macro, FP_ILOGBNAN macro, FP_INFINITE "
      "macro, FP_NAN macro, FP_NORMAL macro,",
      "inline, library, name length, no-return, parameter",
      "prototype, prototype scope, recursive call, return, scope, type, type "
      "conversion",
      "FP_SUBNORMAL macro, FP_ZERO macro, fpclassify macro",
      "general utilities header",
      "general wide string utilities",
      "generic association",
      "generic parameters",
      "generic selection",
      "getc function",
      "getchar function",
      "getenv function",
      "getenv_s function",
      "gets (obsolete)",
      "gets_s function",
      "getwc function",
      "getwchar function",
      "gmtime function",
      "gmtime_s function",
      "goto statement",
      "graphic characters",
      "greater-than operator (>)",
      "greater-than-or-equal-to operator (>=)",
      "grouping structure member",
      "h format modifier",
      "happens before",
      "header",
      "header names",
      "hexadecimal constant",
      "hexadecimal digit",
      "hexadecimal digits (hexadecimal-character escape sequence)",
      "hexadecimal prefix",
      "hexadecimal-character escape sequence (hexadecimal digits)",
      "hh format modifier",
      "hidden",
      "high-order bit",
      "horizontal tab",
      "horizontal-tab character",
      "horizontal-tab escape sequence",
      "hosted",
      "hosted execution environment",
      "HUGE_VAL macro",
      "HUGE_VALF macro",
      "HUGE_VALL macro",
      "hyperbolic functions",
      "hypot function",
      "hypot type-generic macro",
      "hypotf function",
      "hypotl function",
      "I macro",
      "identifier",
      "maximum length",
      "name spaces",
      "reserved",
      "scope",
      "type",
      "identifier list",
      "identifier nondigit",
      "IEC 60559",
      "IEC 559",
      "IEEE floating-point arithmetic standard",
      "IEEE 754",
      "IEEE 854",
      "if preprocessing directive",
      "ifdef",
      "ignore_handler_s function",
      "ilogb function",
      "ilogb type-generic macro",
      "ilogbf function",
      "ilogbl function",
      "imaginary macro",
      "imaginary numbers",
      "imaginary type domain",
      "imaginary types",
      "imaxabs function",
      "imaxdiv function",
      "imaxdiv_t type",
      "implementation",
      "implementation limit",
      "implementation-defined behavior",
      "implementation-defined value",
      "implicit conversion",
      "implicit initialization",
      "inclusive OR operators",
      "bitwise (|)",
      "bitwise assignment (|=)",
      "incomplete",
      "incomplete type",
      "increment operators",
      "indeterminate value",
      "indeterminately sequenced",
      "indirection operator (*)",
      "inequality operator (!=)",
      "infinitary",
      "infinity",
      "INFINITY macro",
      "initial position",
      "initial shift state",
      "initialization, in blocks",
      "initialized",
      "initializer",
      "permitted form",
      "string literal",
      "inline",
      "inline definition",
      "inline function",
      "inner scope",
      "input failure",
      "input/output functions",
      "direct",
      "formatted",
      "input/output header",
      "input/output, device",
      "INT identifier prefix",
      "int type",
      "int type conversion",
      "intN_t types",
      "INTN_C macros",
      "INTN_MAX macros",
      "INTN_MIN macros",
      "int8_t type",
      "INT_FAST identifier prefix",
      "INT_LEAST identifier prefix",
      "int_curr_symbol structure member",
      "INT_FASTN_MAX macros",
      "INT_FASTN_MIN macros",
      "int_fast16_t type",
      "int_fast32_t type",
      "int_fast64_t type",
      "int_fast8_t type",
      "int_fastN_t types",
      "int_frac_digits structure member",
      "INT_LEASTN_MAX macros",
      "INT_LEASTN_MIN macros",
      "int_leastN_t types",
      "int_least16_t type",
      "int_least32_t type",
      "int_least64_t type",
      "int_least8_t type",
      "INT_MAX macro",
      "INT_MIN macro",
      "int_n_cs_precedes structure member",
      "int_n_sep_by_space structure member",
      "int_n_sign_posn structure member",
      "int_p_cs_precedes structure member",
      "int_p_sep_by_space structure member",
      "int_p_sign_posn structure member",
      "integer arithmetic functions",
      "integer character constant",
      "integer constant",
      "integer constant expression",
      "integer conversion rank",
      "integer promotions",
      "integer suffix",
      "integer type conversion",
      "integer types",
      "integer types header",
      "inter-thread happens before",
      "interactive device",
      "internal linkage",
      "internal name",
      "interrupt",
      "INTMAX_C macro",
      "INTMAX_MAX macro",
      "INTMAX_MIN macro",
      "intmax_t type",
      "INTPTR_MAX macro",
      "INTPTR_MIN macro",
      "intptr_t type",
      "is identifier prefix",
      "isalnum function",
      "isalpha function",
      "isblank function",
      "iscntrl function",
      "isdigit function",
      "isfinite macro",
      "isgraph function",
      "isgreater macro",
      "isgreaterequal macro",
      "isinf macro",
      "isless macro",
      "islessequal macro",
      "islessgreater macro",
      "islower function",
      "isnan macro",
      "isnormal macro",
      "ISO/IEC 646",
      "ISO/IEC 9945–2",
      "ISO/IEC 10646",
      "ISO/IEC 10976–1",
      "ISO/IEC 2382",
      "ISO 4217",
      "ISO 80000–2",
      "ISO 8601",
      "isprint function",
      "ispunct function",
      "isspace function",
      "isunordered macro",
      "isupper function",
      "iswalnum function",
      "iswalpha function",
      "iswblank function",
      "iswcntrl function",
      "iswctype function",
      "iswdigit function",
      "iswgraph function",
      "iswlower function",
      "iswprint function",
      "iswpunct function",
      "iswspace function",
      "iswupper function",
      "iswxdigit function",
      "isxdigit function",
      "italic type convention",
      "iteration statements",
      "j format modifier",
      "jmp_buf type",
      "jump statements",
      "keyword",
      "kill_dependency macro",
      "known constant size",
      "L character constant prefix",
      "L format modifier",
      "l identifier suffix",
      "L string literal prefix",
      "L_tmpnam macro",
      "L_tmpnam_s macro",
      "label name",
      "labeled statement",
      "labs function",
      "language",
      "character constant prefix",
      "string literal prefix",
      "syntax summary",
      "Latin alphabet",
      "LC_ identifier prefix",
      "LC_ALL macro",
      "LC_COLLATE macro",
      "LC_CTYPE macro",
      "LC_MONETARY macro",
      "LC_NUMERIC macro",
      "LC_TIME macro",
      "lconv structure type",
      "LDBL_DECIMAL_DIG macro",
      "LDBL_DIG macro",
      "LDBL_EPSILON macro",
      "LDBL_HAS_SUBNORM macro",
      "LDBL_MANT_DIG macro",
      "LDBL_MAX macro",
      "LDBL_MAX_10_EXP macro",
      "LDBL_MAX_EXP macro",
      "LDBL_MIN macro",
      "LDBL_MIN_10_EXP macro",
      "LDBL_MIN_EXP macro",
      "length",
      "left-shift assignment operator (<<=)",
      "left-shift operator (<<)",
      "length modifier",
      "length of a string",
      "length of a wide string",
      "less-than operator (<)",
      "less-than-or-equal-to operator (<=)",
      "letter",
      "lexical elements",
      "lgamma function",
      "lgamma type-generic macro",
      "lgammaf function",
      "lgammal function",
      "library",
      "constant",
      "memory_order_acq_rel",
      "memory_order_acquire",
      "memory_order_consume",
      "memory_order_relaxed",
      "memory_order_release",
      "memory_order_seq_cst",
      "mtx_plain",
      "mtx_recursive",
      "mtx_timed",
      "thrd_busy",
      "thrd_error",
      "thrd_nomem",
      "thrd_success",
      "thrd_timedout",
      "_Exit",
      "abort",
      "abort_handler_s",
      "abs",
      "acos",
      "acosf",
      "acosh",
      "acoshf",
      "acoshl",
      "acosl",
      "aligned_alloc",
      "asctime",
      "asctime_s",
      "asin",
      "asinf",
      "asinh",
      "asinhf",
      "asinhl",
      "asinl",
      "at_quick_exit",
      "at_exit",
      "atexit",
      "atof",
      "atoi",
      "atol",
      "atoll",
      "atomic_compare_exchange_strong",
      "atomic_compare_exchange_strong_explicit",
      "atomic_compare_exchange_weak",
      "atomic_compare_exchange_weak_explicit",
      "atomic_exchange",
      "atomic_exchange_explicit",
      "atomic_fetch_",
      "atomic_flag_clear",
      "atomic_flag_clear_explicit",
      "atomic_flag_test_and_set",
      "atomic_flag_test_and_set_explicit",
      "atomic_init",
      "atomic_is_lock_free",
      "atomic_load",
      "atomic_load_explicit",
      "atomic_signal_fence",
      "atomic_store",
      "atomic_store_explicit",
      "atomic_thread_fence",
      "bsearch",
      "bsearch_s",
      "btowc",
      "c16rtomb",
      "c32rtomb",
      "cabs",
      "cabsf",
      "cabsl",
      "cacos",
      "cacosf",
      "cacosh",
      "cacoshf",
      "cacoshl",
      "cacosl",
      "call_once",
      "calloc",
      "carg",
      "cargf",
      "cargl",
      "casin",
      "casinf",
      "casinh",
      "casinhf",
      "casinhl",
      "casinl",
      "catan",
      "catanf",
      "catanh",
      "catanhf",
      "catanhl",
      "catanl",
      "cbrt",
      "cbrtf",
      "cbrtl",
      "ccos",
      "ccosf",
      "ccosh",
      "ccoshf",
      "ccoshl",
      "ccosl",
      "ceil",
      "ceilf",
      "ceill",
      "cerf",
      "cerfc",
      "cexp",
      "cexp2",
      "cexpf",
      "cexpl",
      "cexpm1",
      "cimag",
      "cimagf",
      "cimagl",
      "clearerr",
      "clgamma",
      "clock",
      "clog",
      "clog10",
      "clog1p",
      "clog2",
      "clogf",
      "clogl",
      "cnd_broadcast",
      "cnd_destroy",
      "cnd_init",
      "cnd_signal",
      "cnd_timedwait",
      "cnd_wait",
      "conj",
      "conjf",
      "conjl",
      "copysign",
      "copysignf",
      "copysignl",
      "cos",
      "cosf",
      "cosh",
      "coshf",
      "coshl",
      "cosl",
      "cpow",
      "cpowf",
      "cpowl",
      "cproj",
      "cprojf",
      "cprojl",
      "creal",
      "crealf",
      "creall",
      "csin",
      "csinf",
      "csinh",
      "csinhf",
      "csinhl",
      "csinl",
      "csqrt",
      "csqrtf",
      "csqrtl",
      "ctan",
      "ctanf",
      "ctanh",
      "ctanhf",
      "ctanhl",
      "ctanl",
      "ctgamma",
      "ctime",
      "ctime_s",
      "difftime",
      "div",
      "erf",
      "erfc",
      "erfcf",
      "erfcl",
      "erff",
      "erfl",
      "exp",
      "exp2",
      "exp2f",
      "exp2l",
      "expf",
      "expl",
      "expm1",
      "expm1f",
      "expm1l",
      "fabs",
      "fabsf",
      "fabsl",
      "fclose",
      "fdim",
      "fdimf",
      "fdiml",
      "feclearexcept",
      "fegetenv",
      "fegetexceptflag",
      "fegetround",
      "feholdexcept",
      "feof",
      "feraiseexcept",
      "ferror",
      "fesetenv",
      "fesetexceptflag",
      "fesetround",
      "fetestexcept",
      "feupdateenv",
      "fflush",
      "fgetc",
      "fgetpos",
      "fgets",
      "fgetwc",
      "fgetws",
      "floor",
      "floorf",
      "floorl",
      "fma",
      "fmaf",
      "fmal",
      "fmax",
      "fmaxf",
      "fmaxl",
      "fmin",
      "fminf",
      "fminl",
      "fmod",
      "fmodf",
      "fmodl",
      "fopen",
      "fopen_s",
      "fprintf_s",
      "fputc",
      "fputs",
      "fputwc",
      "fputws",
      "fread",
      "free",
      "freopen",
      "freopen_s",
      "frexp",
      "frexpf",
      "frexpl",
      "fscanf",
      "fscanf_s",
      "fseek",
      "fsetpos",
      "ftell",
      "fwide",
      "fwprintf",
      "fwprintf_s",
      "fwrite",
      "fwscanf",
      "fwscanf_s",
      "getc",
      "getchar",
      "getenv",
      "getenv_s",
      "gets_s",
      "getwc",
      "getwchar",
      "gmtime_s",
      "hypot",
      "hypotf",
      "hypotl",
      "ignore_handler_s",
      "ilogb",
      "ilogbf",
      "ilogbl",
      "imaxabs",
      "imaxdiv",
      "isalnum",
      "isalpha",
      "isblank",
      "iscntrl",
      "isdigit",
      "isgraph",
      "islower",
      "isprint",
      "ispunct",
      "isspace",
      "isupper",
      "iswalnum",
      "iswalpha",
      "iswblank",
      "iswcntrl",
      "iswctype",
      "iswdigit",
      "iswgraph",
      "iswlower",
      "iswprint",
      "iswpunct",
      "iswspace",
      "iswupper",
      "iswxdigit",
      "isxdigit",
      "labs",
      "ldexp",
      "ldexpf",
      "ldexpl",
      "ldiv",
      "lgamma",
      "lgammaf",
      "lgammal",
      "llabs",
      "lldiv",
      "llrint",
      "llrintf",
      "llrintl",
      "llround",
      "llroundf",
      "llroundl",
      "localeconv",
      "localtime",
      "localtime_s",
      "log",
      "log10",
      "log10f",
      "log10l",
      "log1p",
      "log1pf",
      "log1pl",
      "log2",
      "log2f",
      "log2l",
      "logb",
      "logbf",
      "logbl",
      "logf",
      "logl",
      "longjmp",
      "lrint",
      "lrintf",
      "lrintl",
      "lround",
      "lroundf",
      "lroundl",
      "mblen",
      "mbrlen",
      "mbrtoc16",
      "mbrtoc32",
      "mbrtowc",
      "mbsinit",
      "mbsrtowcs",
      "mbsrtowcs_s",
      "mbstowcs",
      "mbstowcs_s",
      "mbtowc",
      "memchr",
      "memcmp",
      "memcpy",
      "memcpy_s",
      "memmove",
      "memmove_s",
      "memset",
      "memset_s",
      "mktime",
      "modf",
      "modff",
      "modfl",
      "mtx_destroy",
      "mtx_init",
      "mtx_lock",
      "mtx_timedlock",
      "mtx_trylock",
      "mtx_unlock",
      "nan",
      "nanf",
      "nanl",
      "nearbyint",
      "nearbyintf",
      "nearbyintl",
      "nextafter",
      "nextafterf",
      "nextafterl",
      "nexttoward",
      "nexttowardf",
      "nexttowardl",
      "perror",
      "pow",
      "powf",
      "powl",
      "printf_s",
      "puts",
      "putwc",
      "putwchar",
      "qsort",
      "qsort_s",
      "quick_exit",
      "raise",
      "rand",
      "realloc",
      "remainder",
      "remainderf",
      "remainderl",
      "remove",
      "remquo",
      "remquof",
      "remquol",
      "rename",
      "rewind",
      "rint",
      "rintf",
      "rintl",
      "round",
      "roundf",
      "roundl",
      "scalbln",
      "scalblnf",
      "scalblnl",
      "scalbn",
      "scalbnf",
      "scalbnl",
      "scanf",
      "scanf_s",
      "set_constraint_handler_s",
      "setbuf",
      "setjmp",
      "setlocale",
      "setvbuf",
      "signal",
      "sin",
      "sinf",
      "sinh",
      "sinhf",
      "sinhl",
      "sinl",
      "snprintf",
      "snprintf_s",
      "snwprintf_s",
      "sprintf",
      "sprintf_s",
      "sqrt",
      "sqrtf",
      "sqrtl",
      "srand",
      "sscanf",
      "sscanf_s",
      "strcat",
      "strcat_s",
      "strchr",
      "strcmp",
      "strcoll",
      "strcpy",
      "strcpy_s",
      "strcspn",
      "strerror",
      "strerror_s",
      "strerrorlen_s",
      "strftime",
      "strlen",
      "strncat",
      "strncat_s",
      "strncmp",
      "strncpy",
      "strncpy_s",
      "strnlen_s",
      "strpbrk",
      "strrchr",
      "strspn",
      "strstr",
      "strtod",
      "strtof",
      "strtoimax",
      "strtok",
      "strtok_s",
      "strtol",
      "strtold",
      "strtoll",
      "strtoul",
      "strtoull",
      "strtoumax",
      "strxfrm",
      "swprintf",
      "swprintf_s",
      "swscanf",
      "swscanf_s",
      "system",
      "tan",
      "tanf",
      "tanh",
      "tanhf",
      "tanhl",
      "tanl",
      "tgamma",
      "tgammaf",
      "tgammal",
      "thrd_create",
      "thrd_current",
      "thrd_detach",
      "thrd_equal",
      "thrd_exit",
      "thrd_join",
      "thrd_sleep",
      "thrd_yield",
      "timespec_get",
      "tmpfile",
      "tmpfile_s",
      "tmpnam",
      "tmpnam_s",
      "tolower",
      "toupper",
      "towctrans",
      "towlower",
      "towupper",
      "trunc",
      "truncf",
      "truncl",
      "tss_create",
      "tss_delete",
      "tss_get",
      "tss_set",
      "ungetc",
      "ungetwc",
      "va_arg",
      "va_copy",
      "va_end",
      "va_start",
      "vfprintf",
      "vfprintf_s",
      "vfscanf",
      "vfscanf_s",
      "vfwprintf",
      "vfwprintf_s",
      "vfwscanf",
      "vfwscanf_s",
      "vprintf",
      "vprintf_s",
      "vscanf",
      "vscanf_s",
      "vsnprintf",
      "vsnprintf_s",
      "vsnwprintf_s",
      "vsprintf",
      "vsprintf_s",
      "vsscanf",
      "vsscanf_s",
      "vswprintf",
      "vswprintf_s",
      "vswscanf",
      "vswscanf_s",
      "vwprintf",
      "vwprintf_s",
      "vwscanf",
      "vwscanf_s",
      "wcrtomb",
      "wcrtomb_s",
      "wcscat",
      "wcscat_s",
      "wcschr",
      "wcscmp",
      "wcscoll",
      "wcscpy",
      "wcscpy_s",
      "wcscspn",
      "wcsftime",
      "wcslen",
      "wcsncat",
      "wcsncat_s",
      "wcsncmp",
      "wcsncpy",
      "wcsncpy_s",
      "wcsnlen_s",
      "wcspbrk",
      "wcsrchr",
      "wcsrtombs",
      "wcsrtombs_s",
      "wcsspn",
      "wcsstr",
      "wcstod",
      "wcstof",
      "wcstoimax",
      "wcstok",
      "wcstok_s",
      "wcstol",
      "wcstold",
      "wcstoll",
      "wcstoul",
      "wcstoull",
      "wcstoumax",
      "wcsxfrm",
      "wctob",
      "wctomb",
      "wctomb_s",
      "wctrans",
      "wctype",
      "wmemchr",
      "wmemcmp",
      "wmemcpy",
      "wmemcpy_s",
      "wmemmove",
      "wmemmove_s",
      "wmemset",
      "wprintf",
      "wprintf_s",
      "wscanf",
      "wscanf_s",
      "__VA_ARGS__",
      "__func__",
      "errno",
      "identifier prefix",
      "__STDC_",
      "ATOMIC_",
      "cnd_",
      "E",
      "FE_",
      "FP_",
      "INT",
      "INT_FAST",
      "INT_LEAST",
      "is",
      "LC_",
      "mem",
      "memory_",
      "memory_order_",
      "mtx_",
      "PRI",
      "PRId",
      "PRIdFAST",
      "PRIdLEAST",
      "PRIi",
      "PRIiFAST",
      "PRIiLEAST",
      "PRIo",
      "PRIoFAST",
      "PRIoLEAST",
      "PRIu",
      "PRIuFAST",
      "PRIuLEAST",
      "PRIX",
      "PRIXFAST",
      "PRIXLEAST",
      "SCN",
      "SCNd",
      "SCNdFAST",
      "SCNdLEAST",
      "SCNi",
      "SCNiFAST",
      "SCNiLEAST",
      "SCNo",
      "SCNoFAST",
      "SCNoLEAST",
      "SCNu",
      "SCNuFAST",
      "SCNuLEAST",
      "SCNx",
      "SCNxFAST",
      "SCNxLEAST",
      "SIG",
      "str",
      "thrd_",
      "TIME_",
      "tss_",
      "UINT",
      "UINT_FAST",
      "UINT_LEAST",
      "wcs",
      "identifier suffix",
      "_C",
      "_MAX",
      "_MIN",
      "_explicit",
      "MAX",
      "macro",
      "_Complex_I",
      "_IOFBF",
      "_IOLBF",
      "_IONBF",
      "_Imaginary_I",
      "__DATE__",
      "__FILE__",
      "__LINE__",
      "__STDC_ANALYZABLE__",
      "__STDC_HOSTED__",
      "__STDC_IEC_559_COMPLEX__",
      "__STDC_IEC_559__",
      "__STDC_ISO_10646__",
      "__STDC_LIB_EXT1__",
      "__STDC_MB_MIGHT_NEQ_WC__",
      "__STDC_NO_ATOMICS__",
      "__STDC_NO_COMPLEX__",
      "__STDC_NO_THREADS__",
      "__STDC_NO_VLA__",
      "__STDC_UTF_16__",
      "__STDC_UTF_32__",
      "__STDC_VERSION__",
      "__STDC_WANT_LIB_EXT1__",
      "__STDC__",
      "__TIME__",
      "__alignas_is_defined",
      "__alignof_is_defined",
      "__bool_true_false_are_defined",
      "__cplusplus",
      "alignas",
      "alignof",
      "and_eq",
      "assert",
      "ATOMIC_BOOL_LOCK_FREE",
      "ATOMIC_CHAR16_T_LOCK_FREE",
      "ATOMIC_CHAR32_T_LOCK_FREE",
      "ATOMIC_CHAR_LOCK_FREE",
      "ATOMIC_FLAG_INIT",
      "ATOMIC_INT_LOCK_FREE",
      "ATOMIC_LLONG_LOCK_FREE",
      "ATOMIC_LONG_LOCK_FREE",
      "ATOMIC_POINTER_LOCK_FREE",
      "ATOMIC_SHORT_LOCK_FREE",
      "ATOMIC_WCHAR_T_LOCK_FREE",
      "bitand",
      "bitor",
      "BUFSIZ",
      "CHAR_BIT",
      "CHAR_MAX",
      "CHAR_MIN",
      "CLOCKS_PER_SEC",
      "CMPLX",
      "CMPLXF",
      "CMPLXL",
      "compl",
      "complex",
      "DBL_DECIMAL_DIG",
      "DBL_DIG",
      "DBL_EPSILON",
      "DBL_HAS_SUBNORM",
      "DBL_MANT_DIG",
      "DBL_MAX",
      "DBL_MAX_10_EXP",
      "DBL_MAX_EXP",
      "DBL_MIN",
      "DBL_MIN_10_EXP",
      "DBL_MIN_EXP",
      "DBL_TRUE_MIN",
      "DECIMAL_DIG",
      "EDOM",
      "EILSEQ",
      "EOF",
      "ERANGE",
      "EXIT_FAILURE",
      "EXIT_SUCCESS",
      "FALSE",
      "FE_ALL_EXCEPT",
      "FE_DFL_ENV",
      "FE_DIVBYZERO",
      "FE_DOWNWARD",
      "FE_INEXACT",
      "FE_INVALID",
      "FE_OVERFLOW",
      "FE_TONEAREST",
      "FE_TOWARDZERO",
      "FE_UNDERFLOW",
      "FE_UPWARD",
      "FILENAME_MAX",
      "FLT_DECIMAL_DIG",
      "FLT_DIG",
      "FLT_EPSILON",
      "FLT_EVAL_METHOD",
      "FLT_HAS_SUBNORM",
      "FLT_MANT_DIG",
      "FLT_MAX",
      "FLT_MAX_10_EXP",
      "FLT_MAX_EXP",
      "FLT_MIN",
      "FLT_MIN_10_EXP",
      "FLT_MIN_EXP",
      "FLT_RADIX",
      "FLT_ROUNDS",
      "FLT_TRUE_MIN",
      "FOPEN_MAX",
      "FP_FAST_FMA",
      "FP_FAST_FMAF",
      "FP_FAST_FMAL",
      "FP_ILOGB0",
      "FP_ILOGBNAN",
      "FP_INFINITE",
      "FP_NAN",
      "FP_NORMAL",
      "FP_SUBNORMAL",
      "FP_ZERO",
      "fpclassify",
      "HUGE_VAL",
      "HUGE_VALF",
      "HUGE_VALL",
      "INT_MAX",
      "INT_MIN",
      "INTMAX_C",
      "INTMAX_MAX",
      "INTMAX_MIN",
      "INTPTR_MAX",
      "INTPTR_MIN",
      "isfinite",
      "isgreater",
      "isgreaterequal",
      "isinf",
      "isless",
      "islessequal",
      "islessgreater",
      "isnan",
      "isnormal",
      "isunordered",
      "kill_dependency",
      "L_tmpnam",
      "L_tmpnam_s",
      "LC_ALL",
      "LC_COLLATE",
      "LC_CTYPE",
      "LC_MONETARY",
      "LC_NUMERIC",
      "LC_TIME",
      "LDBL_DECIMAL_DIG",
      "LDBL_DIG",
      "LDBL_EPSILON",
      "LDBL_HAS_SUBNORM",
      "LDBL_MANT_DIG",
      "LDBL_MAX",
      "LDBL_MAX_10_EXP",
      "LDBL_MAX_EXP",
      "LDBL_MIN",
      "LDBL_MIN_10_EXP",
      "LDBL_MIN_EXP",
      "LDBL_TRUE_MIN",
      "LLONG_MAX",
      "LLONG_MIN",
      "LONG_MAX",
      "LONG_MIN",
      "MATH_ERREXCEPT",
      "math_errhandling",
      "MATH_ERRNO",
      "MB_CUR_MAX",
      "MB_LEN_MAX",
      "NDEBUG",
      "noreturn",
      "not",
      "not_eq",
      "NULL",
      "offsetof",
      "ONCE_FLAG_INIT",
      "or",
      "or_eq",
      "PRIdFAST32",
      "PRIdMAX",
      "PRIdPTR",
      "PRIiMAX",
      "PRIiPTR",
      "PRIoMAX",
      "PRIoPTR",
      "PRIuMAX",
      "PRIuPTR",
      "PRIXMAX",
      "PRIXPTR",
      "PTRDIFF_MAX",
      "PTRDIFF_MIN",
      "putc",
      "putchar",
      "RAND_MAX",
      "RSIZE_MAX",
      "SCHAR_MAX",
      "SCHAR_MIN",
      "SCNdMAX",
      "SCNdPTR",
      "SCNiMAX",
      "SCNiPTR",
      "SCNoMAX",
      "SCNoPTR",
      "SCNuMAX",
      "SCNuPTR",
      "SCNxMAX",
      "SCNxPTR",
      "SEEK_CUR",
      "SEEK_END",
      "SEEK_SET",
      "SHRT_MAX",
      "SHRT_MIN",
      "SIG_ATOMIC_MAX",
      "SIG_ATOMIC_MIN",
      "SIG_DFL",
      "SIG_ERR",
      "SIG_IGN",
      "SIGABRT",
      "SIGFPE",
      "SIGILL",
      "SIGINT",
      "signbit",
      "SIGSEGV",
      "SIGTERM",
      "SIZE_MAX",
      "static_assert",
      "thread_local",
      "TIME_UTC",
      "TMP_MAX",
      "TMP_MAX_S",
      "TRUE",
      "TSS_DTOR_ITERATIONS",
      "UCHAR_MAX",
      "UINT64_C",
      "UINT_MAX",
      "UINTMAX_C",
      "UINTMAX_MAX",
      "UINTPTR_MAX",
      "ULLONG_MAX",
      "ULONG_MAX",
      "USHRT_MAX",
      "WCHAR_MAX",
      "WCHAR_MIN",
      "WEOF",
      "WINT_MAX",
      "WINT_MIN",
      "xor",
      "xor_eq",
      "obsolete",
      "ATOMIC_VAR_INIT",
      "gets",
      "stream",
      "stderr",
      "stdin",
      "stdout",
      "structure member",
      "currency_symbol",
      "decimal_point",
      "frac_digits",
      "grouping",
      "int_curr_symbol",
      "int_frac_digits",
      "int_n_cs_precedes",
      "int_n_sep_by_space",
      "int_n_sign_posn",
      "int_p_cs_precedes",
      "int_p_sep_by_space",
      "int_p_sign_posn",
      "mon_decimal_point",
      "mon_grouping",
      "mon_thousands_sep",
      "n_cs_precedes",
      "n_sep_by_space",
      "n_sign_posn",
      "negative_sign",
      "p_cs_precedes",
      "p_sep_by_space",
      "p_sign_posn",
      "positive_sign",
      "thousands_sep",
      "tm_isdst",
      "tm_mday",
      "tm_min",
      "tm_mon",
      "tm_sec",
      "tm_wday",
      "tm_yday",
      "tm_year",
      "tv_nsec",
      "tv_sec",
      "structure type",
      "lconv",
      "timespec",
      "tm",
      "summary",
      "terms",
      "_Bool type",
      "_Complex types",
      "_Imaginary types",
      "atomic_bool",
      "atomic_char",
      "atomic_char16_t",
      "atomic_char32_t",
      "atomic_flag",
      "atomic_int",
      "atomic_int_fast16_t",
      "atomic_int_fast32_t",
      "atomic_int_fast64_t",
      "atomic_int_fast8_t",
      "atomic_int_least16_t",
      "atomic_int_least32_t",
      "atomic_int_least64_t",
      "atomic_int_least8_t",
      "atomic_intmax_t",
      "atomic_intptr_t",
      "atomic_llong",
      "atomic_long",
      "atomic_ptrdiff_t",
      "atomic_schar",
      "atomic_short",
      "atomic_size_t",
      "atomic_uchar",
      "atomic_uint",
      "atomic_uint_fast16_t",
      "atomic_uint_fast32_t",
      "atomic_uint_fast64_t",
      "atomic_uint_fast8_t",
      "atomic_uint_least16_t",
      "atomic_uint_least32_t",
      "atomic_uint_least64_t",
      "atomic_uint_least8_t",
      "atomic_uintmax_t",
      "atomic_uintptr_t",
      "atomic_ullong",
      "atomic_ulong",
      "atomic_ushort",
      "atomic_wchar_t",
      "char",
      "char16_t",
      "char32_t",
      "clock_t",
      "cnd_t",
      "constraint_handler_t",
      "div_t",
      "double",
      "double_t",
      "enum",
      "errno_t",
      "fenv_t",
      "fexcept_t",
      "float",
      "float_t",
      "fpos_t",
      "imaxdiv_t",
      "int8_t",
      "int_fast16_t",
      "int_fast32_t",
      "int_fast64_t",
      "int_fast8_t",
      "int_least16_t",
      "int_least32_t",
      "int_least64_t",
      "int_least8_t",
      "intmax_t",
      "intptr_t",
      "jmp_buf",
      "ldiv_t",
      "lldiv_t",
      "max_align_t",
      "mbstate_t",
      "memory_order",
      "mtx_t",
      "once_flag",
      "ptrdiff_t",
      "rsize_t",
      "sig_atomic_t",
      "signed",
      "thrd_start_t",
      "thrd_t",
      "time_t",
      "tss_dtor_t",
      "tss_t",
      "uint_fast16_t",
      "uint_fast32_t",
      "uint_fast64_t",
      "uint_fast8_t",
      "uint_least16_t",
      "uint_least32_t",
      "uint_least64_t",
      "uint_least8_t",
      "uintmax_t",
      "uintptr_t",
      "unsigned",
      "va_list",
      "void",
      "wchar_t",
      "wctrans_t",
      "wctype_t",
      "wint_t",
      "use of functions",
      "lifetime",
      "limits",
      "environmental, see environmental limits",
      "implementation, see implementation limits",
      "numerical, see numerical limits",
      "translation, see translation limits",
      "line buffered",
      "line buffered stream",
      "line number",
      "lines",
      "preprocessing directive",
      "linkage",
      "ll format modifier",
      "llabs function",
      "lldiv function",
      "lldiv_t type",
      "LLONG_MAX macro",
      "LLONG_MIN macro",
      "llrint function",
      "llrint type-generic macro",
      "llrintf function",
      "llrintl function",
      "llround function",
      "llround functions",
      "llround type-generic macro",
      "llroundf function",
      "llroundl function",
      "local time",
      "locale",
      "locale-specific behavior",
      "localeconv function",
      "localization header",
      "localtime function",
      "localtime_s function",
      "log function",
      "log type-generic macro",
      "log10 function",
      "log10 type-generic macro",
      "log10f function",
      "log10l function",
      "log1p function",
      "log1p type-generic macro",
      "log1pf function",
      "log1pl function",
      "log2 function",
      "log2 type-generic macro",
      "log2f function",
      "log2l function",
      "logarithmic functions",
      "real",
      "logb function",
      "logb type-generic macro",
      "logbf function",
      "logbl function",
      "logf function",
      "logical operators",
      "AND (&&)",
      "negation (!)",
      "OR (||)",
      "logical source lines",
      "logl function",
      "long double _Complex type",
      "long double _Complex type conversion",
      "long double _Imaginary type",
      "long double suffix, l or L",
      "long double type",
      "long int type",
      "long int type conversion",
      "long integer suffix, l or L",
      "long long int type",
      "long long int type conversion",
      "long long integer suffix, ll or LL",
      "LONG_MAX macro",
      "LONG_MIN macro",
      "longjmp function",
      "loop body",
      "low-order bit",
      "lowercase letters",
      "lrint function",
      "lrint type-generic macro",
      "lrintf function",
      "lrintl function",
      "lround function",
      "lround functions",
      "lround type-generic macro",
      "lroundf function",
      "lroundl function",
      "lvalue",
      "lvalue conversion",
      "macro argument substitution",
      "macro definition",
      "library function",
      "macro invocation",
      "macro name",
      "predefined",
      "redefinition",
      "macro parameter",
      "macro preprocessor",
      "macro replacement",
      "magnitude, complex",
      "manipulation functions",
      "matching failure",
      "MATH_ERREXCEPT macro",
      "math_errhandling macro",
      "MATH_ERRNO macro",
      "mathematics header",
      "MAX identifier suffix",
      "max_align_t type",
      "maximal munch",
      "maximum functions",
      "MB_CUR_MAX macro",
      "MB_LEN_MAX macro",
      "mblen function",
      "mbrlen function",
      "mbrtoc16 function",
      "mbrtoc32 function",
      "mbrtowc function",
      "mbsinit function",
      "mbsrtowcs function",
      "mbsrtowcs_s function",
      "mbstate_t type",
      "mbstowcs function",
      "mbstowcs_s function",
      "mbtowc function",
      "mem identifier prefix",
      "member access operators (. and-> )",
      "member alignment",
      "members",
      "memchr function",
      "memcmp function",
      "memcpy function",
      "memcpy_s function",
      "memmove function",
      "memmove_s function",
      "memory location",
      "memory management functions",
      "memory_ identifier prefix",
      "memory_order_ identifier prefix",
      "memory_order type",
      "memory_order_acq_rel constant",
      "memory_order_acquire constant",
      "memory_order_consume constant",
      "memory_order_relaxed constant",
      "memory_order_release constant",
      "memory_order_seq_cst constant",
      "memset function",
      "memset_s function",
      "minimum functions",
      "minus operator, unary",
      "miscellaneous functions",
      "mktime function",
      "modf function",
      "modff function",
      "modfl function",
      "modifiable lvalue",
      "modification order",
      "modulus functions",
      "modulus, complex",
      "mon_decimal_point structure member",
      "mon_grouping structure member",
      "mon_thousands_sep structure member",
      "mtx_ identifier prefix",
      "mtx_destroy function",
      "mtx_init function",
      "mtx_lock function",
      "mtx_plain constant",
      "mtx_recursive constant",
      "mtx_t type",
      "mtx_timed constant",
      "mtx_timedlock function",
      "mtx_trylock function",
      "mtx_unlock function",
      "multibyte character",
      "multibyte conversion functions",
      "multibyte string",
      "multibyte/wide character conversion functions",
      "multibyte/wide string conversion functions",
      "multidimensional array",
      "multiplication assignment operator (*=)",
      "multiplication operator (*)",
      "multiplicative expressions",
      "n-char sequence",
      "n-wchar sequence",
      "n_cs_precedes structure member",
      "n_sep_by_space structure member",
      "n_sign_posn structure member",
      "name",
      "external",
      "internal",
      "label",
      "structure/union member",
      "named label",
      "nan function",
      "NAN macro",
      "nanf function",
      "nanl function",
      "NDEBUG macro",
      "nearbyint function",
      "nearbyint type-generic macro",
      "nearbyintf function",
      "nearbyintl function",
      "nearest integer functions",
      "negation operator (!)",
      "negative zero",
      "negative_sign structure member",
      "new line",
      "new-line character",
      "new-line escape sequence",
      "nextafter function",
      "nextafter type-generic macro",
      "nextafterf function",
      "nextafterl function",
      "nexttoward function",
      "nexttoward type-generic macro",
      "nexttowardf function",
      "nexttowardl function",
      "no linkage",
      "no-return function",
      "non-stop floating-point control mode",
      "nongraphic characters",
      "nonlocal jumps header",
      "noreturn macro",
      "norm, complex",
      "normalized",
      "normalized broken-down time",
      "not macro",
      "not-equal-to operator, see inequality operator",
      "not_eq macro",
      "null character",
      "padding of binary stream",
      "NULL macro",
      "null pointer",
      "null pointer constant",
      "null preprocessing directive",
      "null statement",
      "null wide character",
      "number classification macros",
      "numeric conversion functions",
      "O format modifier",
      "object representation",
      "object type",
      "object types",
      "object-like macro",
      "observable behavior",
      "obsolescence",
      "octal constant",
      "octal digit",
      "octal-character escape sequence (octal digits)",
      "OFF pragma",
      "offsetof macro",
      "ON pragma",
      "on-off switch",
      "once_flag type",
      "ONCE_FLAG_INIT macro",
      "ones’ complement",
      "opening",
      "operand",
      "operating system",
      "operations on files",
      "operator",
      "operators",
      "_Alignof operator",
      "additive",
      "assignment",
      "associativity",
      "equality",
      "multiplicative",
      "postfix",
      "precedence",
      "preprocessing",
      "relational",
      "sizeof operator",
      "unary",
      "unary arithmetic",
      "optional features",
      "or macro",
      "OR operators",
      "bitwise exclusive (^)",
      "bitwise exclusive assignment (^=)",
      "bitwise inclusive (|)",
      "bitwise inclusive assignment (|=)",
      "logical (||)",
      "or_eq macro",
      "order of allocated storage",
      "order of evaluation",
      "ordinary identifier name space",
      "orientation",
      "orientation of stream",
      "out-of-bounds store",
      "outer scope",
      "over-aligned",
      "p_cs_precedes structure member",
      "p_sep_by_space structure member",
      "p_sign_posn structure member",
      "padding",
      "binary stream",
      "bits",
      "parameter",
      "array",
      "ellipsis",
      "main function",
      "program",
      "parameter type list",
      "parentheses punctuator (())",
      "parenthesized expression",
      "parse state",
      "perform a trap",
      "permitted form of initializer",
      "perror function",
      "phase angle, complex",
      "physical source lines",
      "placemarker",
      "plus operator, unary",
      "pointer arithmetic",
      "pointer comparison",
      "pointer declarator",
      "pointer operator (->)",
      "pointer to a string",
      "pointer to a wide string",
      "pointer to function",
      "pointer type",
      "pointer type conversion",
      "pointer, null",
      "pole error",
      "portability",
      "position indicator, file",
      "positive difference",
      "positive difference functions",
      "positive_sign structure member",
      "postfix decrement operator (--)",
      "postfix expressions",
      "postfix increment operator (++)",
      "pow function",
      "pow type-generic macro",
      "power functions",
      "powf function",
      "powl function",
      "pp-number",
      "pragma",
      "CX_LIMITED_RANGE",
      "DEFAULT",
      "FENV_ACCESS",
      "FP_CONTRACT",
      "OFF",
      "ON",
      "STDC",
      "pragma operator",
      "pragma preprocessing directive",
      "precedence of operators",
      "precedence of syntax rules",
      "precision",
      "excess",
      "predefined macro names",
      "prefix decrement operator (--)",
      "prefix increment operator (++)",
      "preprocessing concatenation",
      "preprocessing directives",
      "preprocessing file",
      "preprocessing files",
      "preprocessing numbers",
      "preprocessing operators",
      "_Pragma operator",
      "defined operator",
      "preprocessing tokens",
      "preprocessing translation unit",
      "preprocessor",
      "PRI identifier prefix",
      "PRIcFASTN macros",
      "PRIcLEASTN macros",
      "PRIcMAX macros",
      "PRIcN macros",
      "PRIcPTR macros",
      "PRId identifier prefix",
      "PRIdFAST identifier prefix",
      "PRIdFAST32 macro",
      "PRIdLEAST identifier prefix",
      "PRIdMAX macro",
      "PRIdPTR macro",
      "PRIi identifier prefix",
      "PRIiFAST identifier prefix",
      "PRIiLEAST identifier prefix",
      "PRIiMAX macro",
      "PRIiPTR macro",
      "primary expression",
      "printf_s function",
      "printing character",
      "printing wide character",
      "PRIo identifier prefix",
      "PRIoFAST identifier prefix",
      "PRIoLEAST identifier prefix",
      "PRIoMAX macro",
      "PRIoPTR macro",
      "PRIu identifier prefix",
      "PRIuFAST identifier prefix",
      "PRIuLEAST identifier prefix",
      "PRIuMAX macro",
      "PRIuPTR macro",
      "PRIX identifier prefix",
      "PRIXFAST identifier prefix",
      "PRIXLEAST identifier prefix",
      "PRIXMAX macro",
      "PRIXPTR macro",
      "program diagnostics",
      "program execution",
      "program file",
      "program image",
      "program name",
      "program name (argv[0])",
      "program parameters",
      "program startup",
      "program structure",
      "program termination",
      "program, conforming",
      "program, strictly conforming",
      "promotions",
      "default argument",
      "prototype, see function prototype",
      "pseudo-random sequence functions",
      "PTRDIFF_MAX macro",
      "PTRDIFF_MIN macro",
      "ptrdiff_t type",
      "punctuators",
      "putc macro",
      "putchar macro",
      "puts function",
      "putwc function",
      "putwchar function",
      "qsort function",
      "qsort_s function",
      "qualified types",
      "qualified version of type",
      "question-mark escape sequence",
      "quick_exit function",
      "quiet NaN",
      "raise function",
      "rand function",
      "RAND_MAX macro",
      "range excess",
      "range error",
      "rank, see integer conversion rank",
      "read-modify-write operations",
      "read-read coherence",
      "read-write coherence",
      "real floating type conversion",
      "real type domain",
      "real types",
      "real-floating",
      "realloc function",
      "recommended practice",
      "recursion",
      "recursive function call",
      "redefinition of macro",
      "reentrancy",
      "library functions",
      "referenced type",
      "register storage-class specifier",
      "relational expressions",
      "relaxed atomic operations",
      "release fence",
      "release operation",
      "release sequence",
      "reliability of data, interrupted",
      "remainder assignment operator (%=)",
      "remainder function",
      "remainder functions",
      "remainder operator (%)",
      "remainder type-generic macro",
      "remainderf function",
      "remainderl function",
      "remove function",
      "remquo function",
      "remquo type-generic macro",
      "remquof function",
      "remquol function",
      "rename function",
      "representations of types",
      "rescanning and replacement",
      "reserved identifiers",
      "restartable multibyte/wide character conversion functions",
      "restartable multibyte/wide string conversion functions",
      "restore calling environment function",
      "restrict type qualifier",
      "restrict-qualified type",
      "rewind function",
      "right-shift assignment operator (>>=)",
      "right-shift operator (>>)",
      "rint function",
      "rint type-generic macro",
      "rintf function",
      "rintl function",
      "round function",
      "round type-generic macro",
      "roundf function",
      "rounding mode, floating point",
      "roundl function",
      "RSIZE_MAX macro",
      "rsize_t type",
      "runtime-constraint",
      "Runtime-constraint handling functions",
      "rvalue",
      "same scope",
      "save calling environment function",
      "scalar types",
      "scalbln function",
      "scalbln type-generic macro",
      "scalblnf function",
      "scalblnl function",
      "scalbn function",
      "scalbn type-generic macro",
      "scalbnf function",
      "scalbnl function",
      "scanf function",
      "scanf_s function",
      "scanlist",
      "scanset",
      "SCHAR_MAX macro",
      "SCHAR_MIN macro",
      "SCN identifier prefix",
      "SCNcFASTN macros",
      "SCNcLEASTN macros",
      "SCNcMAX macros",
      "SCNcN macros",
      "SCNcPTR macros",
      "SCNd identifier prefix",
      "SCNdFAST identifier prefix",
      "SCNdLEAST identifier prefix",
      "SCNdMAX macro",
      "SCNdPTR macro",
      "SCNi identifier prefix",
      "SCNiFAST identifier prefix",
      "SCNiLEAST identifier prefix",
      "SCNiMAX macro",
      "SCNiPTR macro",
      "SCNo identifier prefix",
      "SCNoFAST identifier prefix",
      "SCNoLEAST identifier prefix",
      "SCNoMAX macro",
      "SCNoPTR macro",
      "SCNu identifier prefix",
      "SCNuFAST identifier prefix",
      "SCNuLEAST identifier prefix",
      "SCNuMAX macro",
      "SCNuPTR macro",
      "SCNx identifier prefix",
      "SCNxFAST identifier prefix",
      "SCNxLEAST identifier prefix",
      "SCNxMAX macro",
      "SCNxPTR macro",
      "scope of identifier",
      "search functions",
      "utility",
      "SEEK_CUR macro",
      "SEEK_END macro",
      "SEEK_SET macro",
      "selection statements",
      "self-referential structure",
      "semicolon punctuator (;)",
      "separate compilation",
      "separate translation",
      "sequence points",
      "sequenced after, see sequenced before",
      "sequenced before",
      "sequencing of statements",
      "sequential consistency",
      "set_constraint_handler_s function",
      "setbuf function",
      "setjmp function",
      "setlocale function",
      "setvbuf function",
      "shall",
      "shift expressions",
      "shift sequence",
      "shift states",
      "short identifier, character",
      "short int type",
      "short int type conversion",
      "SHRT_MAX macro",
      "SHRT_MIN macro",
      "side effects",
      "SIG identifier prefix",
      "SIG_ATOMIC_MAX macro",
      "SIG_ATOMIC_MIN macro",
      "sig_atomic_t type",
      "SIG_DFL macro",
      "SIG_ERR macro",
      "SIG_IGN macro",
      "SIGABRT macro",
      "SIGFPE macro",
      "SIGILL macro",
      "SIGINT macro",
      "sign and magnitude",
      "sign bit",
      "signal function",
      "signal handler",
      "signal handling functions",
      "signal handling header",
      "signaling NaN",
      "signals",
      "signbit macro",
      "signed char type",
      "signed character",
      "signed integer types",
      "signed type",
      "signed type conversion",
      "signed types",
      "significand part",
      "SIGSEGV macro",
      "SIGTERM macro",
      "simple assignment",
      "simple assignment operator (=)",
      "sin function",
      "sin type-generic macro",
      "sinf function",
      "single-byte character",
      "single-byte/wide character conversion functions",
      "single-precision arithmetic",
      "single-quote escape sequence",
      "singularity",
      "sinh function",
      "sinh type-generic macro",
      "sinhf function",
      "sinhl function",
      "sinl function",
      "SIZE_MAX macro",
      "size_t type",
      "sizes of integer types header",
      "snprintf function",
      "snprintf_s function",
      "snwprintf_s function",
      "sorting utility functions",
      "source character set",
      "source file",
      "source file inclusion",
      "source lines",
      "source text",
      "space character (’ ’)",
      "space format flag",
      "spilling",
      "sprintf function",
      "sprintf_s function",
      "sqrt function",
      "sqrt type-generic macro",
      "sqrtf function",
      "sqrtl function",
      "srand function",
      "sscanf function",
      "sscanf_s function",
      "standard error stream",
      "standard headers",
      "standard input stream",
      "standard integer types",
      "standard output stream",
      "standard signed integer types",
      "standard unsigned integer types",
      "state-dependent encoding",
      "statement",
      "statements",
      "break",
      "compound",
      "do",
      "else",
      "for",
      "goto",
      "if",
      "iteration",
      "jump",
      "labeled",
      "return",
      "selection",
      "sequencing",
      "switch",
      "while",
      "static assertions",
      "static storage duration",
      "static storage-class specifier",
      "static, in array declarators",
      "static_assert declaration",
      "static_assert macro",
      "STDC pragma",
      "stderr stream",
      "stdin stream",
      "stdout stream",
      "storage duration",
      "storage order of array",
      "storage unit (bit-field)",
      "storage-class specifiers",
      "store and load",
      "str identifier prefix",
      "strcat function",
      "strcat_s function",
      "strchr function",
      "strcmp function",
      "strcoll function",
      "strcpy function",
      "strcpy_s function",
      "strcspn function",
      "streams",
      "standard error",
      "standard input",
      "standard output",
      "unbuffered",
      "strerror function",
      "strerror_s function",
      "strerrorlen_s function",
      "strftime function",
      "stricter",
      "strictly conforming program",
      "library function conventions",
      "literal",
      "string handling header",
      "stringizing",
      "strlen function",
      "strncat function",
      "strncat_s function",
      "strncmp function",
      "strncpy function",
      "strncpy_s function",
      "strnlen_s function",
      "stronger",
      "strpbrk function",
      "strrchr function",
      "strspn function",
      "strstr function",
      "strtod function",
      "strtof function",
      "strtoimax function",
      "strtok function",
      "strtok_s function",
      "strtol function",
      "strtold function",
      "strtoll function",
      "strtoul function",
      "strtoull function",
      "strtoumax function",
      "struct hack",
      "structure",
      "arrow operator (->)",
      "content",
      "dot operator (.)",
      "initialization",
      "member name space",
      "member operator (.)",
      "specifier",
      "tag",
      "structure content",
      "strxfrm function",
      "subnormal floating-point numbers",
      "subscripting",
      "subtraction assignment operator (-=)",
      "subtraction operator (-)",
      "suffix",
      "switch body",
      "switch case label",
      "switch default label",
      "switch statement",
      "swprintf function",
      "swprintf_s function",
      "swscanf function",
      "swscanf_s function",
      "symbols",
      "synchronization operation",
      "synchronize with",
      "syntactic categories",
      "syntax notation",
      "syntax rule precedence",
      "syntax summary, language",
      "system function",
      "t format modifier",
      "tab characters",
      "tag compatibility",
      "tag name space",
      "tags",
      "tan function",
      "tan type-generic macro",
      "tanf function",
      "tanh function",
      "tanh type-generic macro",
      "tanhf function",
      "tanhl function",
      "tanl function",
      "temporary lifetime",
      "tentative definition",
      "text streams",
      "thousands_sep structure member",
      "thrd_ identifier prefix",
      "thrd_busy constant",
      "thrd_create function",
      "thrd_current function",
      "thrd_detach function",
      "thrd_equal function",
      "thrd_error constant",
      "thrd_exit function",
      "thrd_join function",
      "thrd_nomem constant",
      "thrd_sleep function",
      "thrd_start_t type",
      "thrd_success constant",
      "thrd_t type",
      "thrd_timedout constant",
      "thrd_yield function",
      "thread",
      "thread of execution",
      "thread storage duration",
      "thread_local macro",
      "threads header",
      "broken down",
      "calendar",
      "components",
      "local",
      "normalized broken down",
      "time base",
      "time function",
      "TIME_ identifier prefix",
      "time_t type",
      "TIME_UTC macro",
      "timespec structure type",
      "timespec_get function",
      "tm structure type",
      "tm_hour structure member",
      "tm_isdst structure member",
      "tm_mday structure member",
      "tm_min structure member",
      "tm_mon structure member",
      "tm_sec structure member",
      "tm_wday structure member",
      "tm_yday structure member",
      "tm_year structure member",
      "TMP_MAX macro",
      "TMP_MAX_S macro",
      "tmpfile function",
      "tmpfile_s function",
      "tmpnam function",
      "tmpnam_s function",
      "to identifier prefix",
      "token",
      "token concatenation",
      "token pasting",
      "tolower function",
      "toupper function",
      "towctrans function",
      "towlower function",
      "towupper function",
      "translation environment",
      "translation limits",
      "translation phases",
      "translation unit",
      "trap",
      "trap representation",
      "trigonometric functions (complex)",
      "trigonometric functions (real)",
      "trigraph sequences",
      "true macro",
      "trunc function",
      "trunc type-generic macro",
      "truncation",
      "truncation toward zero",
      "truncf function",
      "truncl function",
      "tss_ identifier prefix",
      "tss_create function",
      "tss_delete function",
      "TSS_DTOR_ITERATIONS macro",
      "tss_dtor_t type",
      "tss_get function",
      "tss_set function",
      "tss_t type",
      "tv_nsec structure member",
      "tv_sec structure member",
      "two’s complement",
      "type category",
      "type conversion",
      "type definitions",
      "type domain",
      "type name",
      "type names",
      "type punning",
      "type qualifiers",
      "type specifiers",
      "type-generic macro",
      "type-generic math header",
      "typedef declaration",
      "typedef storage-class specifier",
      "types",
      "atomic",
      "compatible",
      "composite",
      "const qualified",
      "conversions",
      "restrict qualified",
      "volatile qualified",
      "U character constant prefix",
      "U string literal prefix",
      "u8 string literal prefix",
      "UCHAR_MAX macro",
      "UINT identifier prefix",
      "UINTN_C macros",
      "UINTN_MAX macros",
      "uintN_t types",
      "UINT64_C macro",
      "UINT_FAST identifier prefix",
      "UINT_LEAST identifier prefix",
      "UINT_FASTN_MAX macros",
      "uint_fast16_t type",
      "uint_fast32_t type",
      "uint_fast64_t type",
      "uint_fast8_t type",
      "uint_fastN_t types",
      "UINT_LEASTN_MAX macros",
      "uint_leastN_t types",
      "uint_least16_t type",
      "uint_least32_t type",
      "uint_least64_t type",
      "uint_least8_t type",
      "UINT_MAX macro",
      "UINTMAX_C macro",
      "UINTMAX_MAX macro",
      "uintmax_t type",
      "UINTPTR_MAX macro",
      "uintptr_t type",
      "ULLONG_MAX macro",
      "ULONG_MAX macro",
      "unary arithmetic operators",
      "unary expression",
      "unary minus operator (-)",
      "unary operators",
      "unary plus operator (+)",
      "unbuffered stream",
      "undef",
      "undefined behavior",
      "underscore character",
      "underscore, leading, in identifier",
      "ungetc function",
      "ungetwc function",
      "Unicode",
      "Unicode required set",
      "unicode utilities header",
      "union",
      "union content",
      "universal character name",
      "unnormalized floating-point numbers",
      "unqualified type",
      "unqualified version of type",
      "unsequenced",
      "unsigned integer suffix (u or U)",
      "unsigned integer types",
      "unsigned type",
      "unsigned type conversion",
      "unsigned types",
      "unspecified behavior",
      "unspecified value",
      "unsuccessful termination",
      "uppercase letters",
      "use of library functions",
      "USHRT_MAX macro",
      "usual arithmetic conversions",
      "UTF–16",
      "UTF–32",
      "UTF–8 string literal",
      "utilities, general",
      "utilities, unicode",
      "va_arg function",
      "va_copy function",
      "va_end function",
      "va_list type",
      "va_start function",
      "value",
      "value bits",
      "value of a string",
      "value of a wide string",
      "variable arguments",
      "variable arguments header",
      "variable length array",
      "variably modified",
      "variably modified type",
      "vertical tab",
      "vertical-tab character",
      "vertical-tab escape sequence",
      "vfprintf function",
      "vfprintf_s function",
      "vfscanf function",
      "vfscanf_s function",
      "vfwprintf function",
      "vfwprintf_s function",
      "vfwscanf function",
      "vfwscanf_s function",
      "visibility of identifier",
      "visible",
      "visible side effect",
      "VLA (variable length array)",
      "void expression",
      "void function parameter",
      "void type conversion",
      "volatile storage",
      "volatile type qualifier",
      "volatile-qualified type",
      "vprintf function",
      "vprintf_s function",
      "vscanf function",
      "vscanf_s function",
      "vsnprintf function",
      "vsnprintf_s function",
      "vsnwprintf_s function",
      "vsprintf function",
      "vsprintf_s function",
      "vsscanf function",
      "vsscanf_s function",
      "vswprintf function",
      "vswprintf_s function",
      "vswscanf function",
      "vswscanf_s function",
      "vwprintf function",
      "vwprintf_s function",
      "vwscanf function",
      "vwscanf_s function",
      "warnings",
      "WCHAR_MAX macro",
      "WCHAR_MIN macro",
      "wchar_t type",
      "wcrtomb function",
      "wcrtomb_s function",
      "wcs identifier prefix",
      "wcscat function",
      "wcscat_s function",
      "wcschr function",
      "wcscmp function",
      "wcscoll function",
      "wcscpy function",
      "wcscpy_s function",
      "wcscspn function",
      "wcsftime function",
      "wcslen function",
      "wcsncat function",
      "wcsncat_s function",
      "wcsncmp function",
      "wcsncpy function",
      "wcsncpy_s function",
      "wcsnlen_s function",
      "wcspbrk function",
      "wcsrchr function",
      "wcsrtombs function",
      "wcsrtombs_s function",
      "wcsspn function",
      "wcsstr function",
      "wcstod function",
      "wcstof function",
      "wcstoimax function",
      "wcstok function",
      "wcstok_s function",
      "wcstol function",
      "wcstold function",
      "wcstoll function",
      "wcstombs function",
      "wcstombs_s function",
      "wcstoul function",
      "wcstoull function",
      "wcstoumax function",
      "wcsxfrm function",
      "wctob function",
      "wctomb function",
      "wctomb_s function",
      "wctrans function",
      "wctrans_t type",
      "wctype function",
      "wctype_t type",
      "weaker",
      "WEOF macro",
      "white space",
      "white-space characters",
      "formatted input/output functions",
      "input functions",
      "output functions",
      "single-byte conversion functions",
      "wide character classification and mapping utilities header",
      "wide character input functions",
      "wide character input/output functions",
      "wide character output functions",
      "wide string comparison functions",
      "wide string concatenation functions",
      "wide string copying functions",
      "wide string literal",
      "wide string miscellaneous functions",
      "wide string numeric conversion functions",
      "wide string search functions",
      "wide-oriented stream",
      "width",
      "WINT_MAX macro",
      "WINT_MIN macro",
      "wint_t type",
      "wmemchr function",
      "wmemcmp function",
      "wmemcpy function",
      "wmemcpy_s function",
      "wmemmove function",
      "wmemmove_s function",
      "wmemset function",
      "wprintf function",
      "wprintf_s function",
      "write-read coherence",
      "write-write coherence",
      "wscanf function",
      "wscanf_s function",
      "xor macro",
      "xor_eq macro",
      "z format modifier",
      "zero"};

  std::vector<std::string> optLevel = {"-O0", "-O1",    "-O2", "-O3",
                                       "-Os", "-Ofast", "-Og", "-Oz"};
  std::vector<std::string> compilerFlags = {
      // Optimization
      "-ftree-vectorize", "-fno-tree-vectorize", "-foptimize-sibling-calls",
      "-fomit-frame-pointer", "-fno-omit-frame-pointer", "-ffunction-sections",
      "-fdata-sections", "-fno-inline", "-finline", "-finline-functions",
      "-fno-inline-functions", "-flto", "-fno-lto", "-freorder-blocks",
      "-freorder-blocks-algorithm=simple", "-freorder-blocks-algorithm=stc",
      "-freorder-functions", "-fno-reorder-functions", "-ffast-math",
      "-fno-fast-math", "-funsafe-math-optimizations", "-ffinite-math-only",

      // Warnings
      "-Wall", "-Wextra", "-Werror", "-Wpedantic", "-Wunused", "-Wconversion",
      "-Wshadow", "-Wcast-align", "-Wcast-qual", "-Wdouble-promotion",
      "-Wformat=2", "-Wformat-security", "-Wformat-signedness", "-Wlogical-op",
      "-Wmissing-include-dirs", "-Wno-unused-parameter", "-Woverloaded-virtual",
      "-Wsign-conversion", "-Wstrict-overflow=5", "-Wundef", "-Wuninitialized",
      "-Wunused-function", "-Wunused-label", "-Wunused-value",
      "-Wunused-variable", "-Wdate-time", "-Wfatal-errors", "-Wfloat-equal",
      "-Winit-self", "-Winline", "-Wmissing-declarations",
      "-Wmissing-field-initializers", "-Wnull-dereference", "-Wold-style-cast",
      "-Wpointer-arith", "-Wredundant-decls", "-Wreorder", "-Wreturn-type",
      "-Wsign-promo", "-Wstack-protector", "-Wstrict-aliasing=3",
      "-Wswitch-default", "-Wswitch-enum", "-Wtrampolines", "-Wvla",
      "-Wwrite-strings", "-Warray-bounds=2", "-Wattribute-alias=2",
      "-Wformat-overflow=2", "-Wformat-truncation=2", "-Wshift-overflow=2",
      "-Wimplicit-fallthrough=5", "-Wduplicated-branches", "-Wduplicated-cond",
      "-Wno-pragmas", "-Wno-unknown-pragmas", "-Wno-attributes",

      // Debugging
      "-g", "-ggdb", "-g3", "-ggdb3", "-gdwarf-2", "-gdwarf-3", "-gdwarf-4",
      "-gdwarf-5", "-grecord-gcc-switches", "-gno-record-gcc-switches",
      "-gcolumn-info", "-gno-column-info", "-gstrict-dwarf",
      "-gno-strict-dwarf", "-gstabs", "-gstabs+", "-gxcoff", "-gxcoff+",
      "-gcoff", "-gcoff+", "-fdebug-prefix-map", "-fdebug-types-section",

      // Architecture
      "-fPIC", "-fpic", "-fPIE", "-fpie", "-m32", "-m64", "-mx32",
      "-march=native", "-mtune=native", "-msse", "-msse2", "-msse3", "-mssse3",
      "-msse4", "-msse4.1", "-msse4.2", "-mavx", "-mavx2", "-mavx512f", "-mfma",
      "-mno-fma", "-mfma4", "-mno-fma4", "-maes", "-mno-aes", "-mpclmul",
      "-mno-pclmul", "-mfsgsbase", "-mno-fsgsbase", "-mrdrnd", "-mno-rdrnd",
      "-mf16c", "-mno-f16c", "-mfentry", "-mno-fentry", "-m3dnow", "-m3dnow-a",
      "-mabi=sysv", "-mabi=ms", "-masm=intel", "-masm=att", "-mcmodel=small",
      "-mcmodel=kernel", "-mcmodel=medium", "-mcmodel=large",
      "-mstack-alignment=4", "-mstack-alignment=8", "-mstack-alignment=16",
      "-mstackrealign", "-mno-stackrealign",

      // Standards
      "-std=c89", "-std=c90", "-std=c99", "-std=c11", "-std=c17", "-std=c2x",
      "-std=gnu89", "-std=gnu90", "-std=gnu99", "-std=gnu11", "-std=gnu17",
      "-std=gnu2x", "-std=c++98", "-std=c++03", "-std=c++11", "-std=c++14",
      "-std=c++17", "-std=c++20", "-std=c++23", "-std=gnu++98", "-std=gnu++03",
      "-std=gnu++11", "-std=gnu++14", "-std=gnu++17", "-std=gnu++20",
      "-std=gnu++23", "-ansi",

      // Security
      "-fstack-protector", "-fstack-protector-all", "-fstack-protector-strong",
      "-fstack-protector-explicit", "-fstack-clash-protection",
      "-fcf-protection=full", "-fcf-protection=return",
      "-fcf-protection=branch", "-fno-cf-protection", "-D_FORTIFY_SOURCE=1",
      "-D_FORTIFY_SOURCE=2", "-D_FORTIFY_SOURCE=3", "-fno-common",
      "-fstack-check", "-fvtable-verify=none", "-fvtable-verify=preinit",
      "-fvtable-verify=std", "-mindirect-branch=thunk",
      "-mindirect-branch=thunk-inline", "-mindirect-branch=thunk-extern",
      "-mfunction-return=thunk", "-mfunction-return=thunk-inline",
      "-mfunction-return=thunk-extern",

      // Linking
      "-static", "-shared", "-pipe", "-pthread", "-static-libgcc",
      "-static-libstdc++", "-rdynamic", "-s", "-no-pie", "-pie",
      "-nostartfiles", "-nodefaultlibs", "-nostdlib", "-nostdinc",
      "-nostdinc++", "-fvisibility=default", "-fvisibility=internal",
      "-fvisibility=hidden", "-fvisibility=protected", "-fwhole-program",
      "-fno-whole-program",

      // Sanitizers and Analysis
      "-fsanitize=address", "-fsanitize=thread", "-fsanitize=undefined",
      "-fsanitize=memory", "-fsanitize=leak", "-fsanitize=kernel-address",
      "-fsanitize=pointer-compare", "-fsanitize=pointer-subtract",
      "-fsanitize=bounds", "-fsanitize=bounds-strict", "-fsanitize=alignment",
      "-fsanitize=object-size", "-fsanitize=float-divide-by-zero",
      "-fsanitize=float-cast-overflow", "-fsanitize=returns-nonnull-attribute",
      "-fsanitize=nonnull-attribute", "-fsanitize=bool", "-fsanitize=enum",
      "-fsanitize=vptr", "-fsanitize=signed-integer-overflow", "-fanalyzer",
      "-fprofile-arcs", "-ftest-coverage", "-fprofile-generate",
      "-fprofile-use", "-fprofile-values", "-fprofile-update=atomic",
      "-fprofile-update=prefer-atomic", "-fprofile-partial-training",
      "-fprofile-correction", "-fprofile-reorder-functions",
      "-fprofile-sample-use", "-fauto-profile", "-fcs-profile-generate",
      "-fcs-profile-use",

      // Miscellaneous
      "-save-temps", "-save-temps=obj", "-fdump-tree-all", "-v", "-H", "-MMD",
      "-MP", "-fmax-errors=N", "-fdiagnostics-color=always",
      "-fdiagnostics-color=never", "-fdiagnostics-color=auto",
      "-fdiagnostics-show-option", "-fdiagnostics-show-location=once",
      "-fdiagnostics-show-location=every-line", "-fverbose-asm",
      "-fno-verbose-asm", "-fshow-column", "-fno-show-column",
      "-fdirectives-only", "-fpreprocessed", "-traditional", "-traditional-cpp",
      "-trigraphs", "-no-integrated-cpp", "-frecord-gcc-switches",
      "-frandom-seed", "-fsched-pressure", "-fsched-spec", "-fsched-spec-load",
      "-fsched-stalled-insns", "-fsched-stalled-insns-dep", "-fsection-anchors",
      "-fstack-usage", "-ftime-report", "-ftime-report-details",
      "-fvar-tracking", "-fvar-tracking-assignments",
      "-fvariable-expansion-in-unroller", "-fweb", "-fwhole-file", "-fwpa",
      "-fwrapv", "-fzero-initialized-in-bss", "-fwritable-strings",
      "-fleading-underscore", "-ftoplevel-reorder", "-ftree-reassoc",
      "-ftree-pre", "-ftree-partial-pre", "-ftree-loop-distribute-patterns",
      "-ftree-loop-distribution", "-ftree-loop-if-convert",
      "-ftree-loop-if-convert-stores", "-ftree-loop-im", "-ftree-loop-ivcanon",
      "-ftree-loop-linear", "-ftree-loop-optimize",
      "-ftree-parallelize-loops=n", "-ftree-phiprop", "-ftree-pta",
      "-ftree-reassoc", "-ftree-sink", "-ftree-slsr", "-ftree-sra",
      "-ftree-switch-conversion", "-ftree-tail-merge", "-ftree-ter",
      "-ftree-vectorizer-verbose=n", "-ftree-vrp"};

  std::vector<std::string> llvmPasses;
  std::mt19937 rng;

  std::vector<std::string> split_passes(const std::string &passes_string) {
    std::vector<std::string> parts;
    std::string temp_part;
    int nesting_level = 0;

    for (char c : passes_string) {
      if (c == '(' || c == '<') {
        nesting_level++;
      } else if (c == ')' || c == '>') {
        nesting_level--;
      }

      if (c == ',' && nesting_level == 0) {
        while (!temp_part.empty() && std::isspace(temp_part.back())) {
          temp_part.pop_back();
        }
        size_t start = temp_part.find_first_not_of(" \t\n\r");
        if (start != std::string::npos && !temp_part.empty()) {
          parts.push_back(temp_part.substr(start));
        }
        temp_part.clear();
      } else {
        temp_part += c;
      }
    }

    while (!temp_part.empty() && std::isspace(temp_part.back())) {
      temp_part.pop_back();
    }
    size_t start = temp_part.find_first_not_of(" \t\n\r");
    if (start != std::string::npos && !temp_part.empty()) {
      parts.push_back(temp_part.substr(start));
    }

    parts.erase(std::remove_if(parts.begin(), parts.end(),
                               [](const std::string &pass) {
                                 return pass == "BitcodeWriterPass";
                               }),
                parts.end());

    return parts;
  }

  std::string exec(const char *cmd) {
    std::array<char, 128> buffer;
    std::string result;
    std::unique_ptr<FILE, decltype(&pclose)> pipe(popen(cmd, "r"), pclose);

    if (!pipe) {
      throw std::runtime_error("popen() failed!");
    }

    while (fgets(buffer.data(), buffer.size(), pipe.get()) != nullptr) {
      result += buffer.data();
    }
    return result;
  }

  /** Instead of manually filling tokens for middle-end
   * optimisation tokens we are now getting the list of
   * passes available for the middle-end pipeline via
   * llvm-opt command.
   * */
  void initializeLLVMPasses() {
    std::vector<std::string> opt_levels = {"O0", "O1", "O2", "O3"};
    std::vector<std::string> pass_types = {"default", "lto-pre-link", "lto"};

    for (const auto &opt : opt_levels) {
      for (const auto &type : pass_types) {
        try {
          std::string cmd = "opt -passes='" + type + "<" + opt +
                            ">' "
                            "-print-pipeline-passes /dev/null 2>/dev/null";
          std::string result = exec(cmd.c_str());
          auto passes = split_passes(result);
          llvmPasses.insert(llvmPasses.end(), passes.begin(), passes.end());
        } catch (const std::runtime_error &e) {
          std::cerr << "Error collecting passes for " << type << "<" << opt
                    << ">: " << e.what() << std::endl;
        }
      }
    }
  }

  template <typename T> T getRandomElement(const std::vector<T> &vec) {
    if (vec.empty())
      return T();

    std::uniform_int_distribution<size_t> dist(0, vec.size() - 1);
    return vec[dist(rng)];
  }

public:
  LLMTokensOption()
      : rng(std::chrono::steady_clock::now().time_since_epoch().count()) {
    std::cout << "Initializing LLVM passes..." << std::endl;

    // Check if opt exists
    FILE *pipe = popen("which opt", "r");
    if (!pipe) {
      std::cerr << "Error: Cannot check for opt command" << std::endl;
      return;
    }
    std::array<char, 128> buffer;
    std::string opt_path;
    while (fgets(buffer.data(), buffer.size(), pipe) != nullptr) {
      opt_path += buffer.data();
    }
    pclose(pipe);

    if (opt_path.empty()) {
      std::cerr << "Warning: 'opt' command not found in PATH" << std::endl;
      return;
    }

    std::cout << "Found opt at: " << opt_path << std::endl;
    initializeLLVMPasses();
  }
  std::string getRandomCompilerOpt() { return getRandomElement(llvmPasses); }
  std::string getRandomCompilerParts() {
    return getRandomElement(compilerParts);
  }
  std::string getRandomOptLevel() { return getRandomElement(optLevel); }
  std::string getRandomCompilerFlag() {
    return getRandomElement(compilerFlags);
  }
  std::string getRandomPL() { return getRandomElement(programmingLanguage); }

  std::string getRandomLLVMPass() {
    for (auto pass : llvmPasses)
      std::cout << "pass: " << pass << std::endl;
    return getRandomElement(llvmPasses);
  }
};

#endif
